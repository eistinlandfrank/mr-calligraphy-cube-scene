...
assert(linkCopyReportReceipt.receipt.urlDigest.match(/^[a-f0-9]{64}$/), "报告链接复制回执应包含链接摘要。");
assert(linkCopyReportReceipt.receipt.receiptDigest.match(/^[a-f0-9]{64}$/), "报告链接复制回执应包含回执摘要。");
...
assert(linkCopyHistoryReceipt.receipt.urlDigest.match(/^[a-f0-9]{64}$/), "档案链接复制回执应包含链接摘要。");
assert(linkCopyHistoryReceipt.receipt.receiptDigest.match(/^[a-f0-9]{64}$/), "档案链接复制回执应包含回执摘要。");
...
assert(localLinkCopyAudit.verifiedCount >= 2, "本机链接复制审计应统计本机校验通过数量。");
assert(localLinkCopyAudit.failedCount === 0, "本机链接复制审计不应出现摘要不匹配。");
assert(localLinkCopyAudit.receipts.every((receipt) => receipt.verificationStatus === "verified"), "本机链接复制回执应通过 receiptDigest 重算校验。");
assert(localLinkCopyAudit.receipts.every((receipt) => receipt.verificationExpectedDigest === receipt.receiptDigest), "本机链接复制回执重算摘要应匹配 receiptDigest。");
...
assert(localLinkCopyAuditExport.html.includes("本机校验通过"), "本机链接复制审计 HTML 应包含本机校验状态。");
assert(localLinkCopyAuditExport.html.includes("重算摘要"), "本机链接复制审计 HTML 应包含重算摘要。");
...
async function runHistoryRepositoryMockServerChecks(fetchApi) {
  ...
  assert(historyRepositoryExportAudit.verifiedCount === 1, "学习档案仓库导出审计应统计本机校验通过数量。");
  assert(historyRepositoryExportAudit.failedCount === 0, "学习档案仓库导出正常回执不应出现摘要失败。");
  assert(historyRepositoryExportAudit.receipts[0].verificationStatus === "verified", "学习档案仓库导出回执应通过 receiptDigest 重算校验。");
  assert(historyRepositoryExportAudit.receipts[0].verificationExpectedDigest === historyRepositoryExportReceipt.receipt.receiptDigest, "学习档案仓库导出回执重算摘要应匹配 receiptDigest。");
  ...
  assert(historyRepositoryExportAuditExport.html.includes("本机校验通过"), "学习档案仓库导出审计 HTML 应包含本机校验结果。");
  assert(historyRepositoryExportAuditExport.html.includes("重算摘要"), "学习档案仓库导出审计 HTML 应包含重算摘要。");
