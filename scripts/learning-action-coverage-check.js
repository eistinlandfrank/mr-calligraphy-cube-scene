...
if (!sceneActions.length) {
  failures.push("未能从 SCENES 中解析到 VR 菜单动作。");
}

actionLabels.forEach((label) => {
  if (!featureLabels.has(label)) {
    failures.push(`VR 菜单动作缺少 LEARNING_ACTION_FEATURES 标记：${label}`);
  }
  if (!handlerLabels.has(label)) {
    failures.push(`VR 菜单动作缺少 runLearningAction 处理分支：${label}`);
  }
});

featureEntries.forEach((entry) => {
  if (!VALID_STATES.has(entry.state)) {
    failures.push(`VR 菜单动作状态不应为 ${entry.state}：${entry.label}`);
  }
});
...
if (failures.length) {
  console.error("VR 菜单动作覆盖检查失败：");
  failures.forEach((failure) => console.error(`- ${failure}`));
  process.exit(1);
}

console.log(`VR 菜单动作覆盖检查通过：${countScenes(source)} 个场景，${actionLabels.length} 个动作，${featureEntries.length} 个状态标记，${handlerCases.length} 个处理分支。`);
