...
- VR 分阶段交互菜单：首页只显示 3 个一级方向；确认后只显示 3 个二级动作；进入练字、报告、档案或计划后菜单自动收起，只保留当前功能最多 3 个口语化操作。
- 适老化前台主面板：22px 动作词标题、18px 大按钮、短动作词；左上状态胶囊、说明小字和点阵拖拽柄不占用老人默认视野，书写与返回工具按钮不小于 44px。
- MR / AR / VR 三种空间表现模式。
