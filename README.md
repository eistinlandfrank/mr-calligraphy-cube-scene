...
- 基础场景桌面内置隐藏写实书写层，进入书写台时显示宣纸、后台可控的独立“永”字墨迹、毛笔、砚台、印章和透明讲解屏。
- 书写练习画布使用自绘白底宣纸、浅色“永”字底稿和 ResizeObserver，切入写字时会重新适配真实尺寸，避免红色透明框被 3D 背景压暗。
- `main-admin.html` 左侧改为多场景物件控制，可切换基础场景、写字场景、展示场景和新增/导入；写字宣纸、永字墨迹、砚台、毛笔、印章和讲解屏都能用同一套坐标控件保存到本机布局。
...
- 老人结果页反馈直接读取本机真实数据，显示“已有 N 条 / 作品 N 幅 / 均分 N / 计划 N/N”等短词，不再用空泛的“已完成”代替结果。
- 前台按钮按真实条件显示：空白练习格不显示撤销、清空、回放；没有笔迹不显示保存作品；没有作品不显示查看作品、导出分享页或优秀筛选，避免老人点到无效按钮。
...
- `npm run build` 内置老人前台可见文字审计，会自动打开手机/桌面、两级菜单、真实数据页、强制误开面板和确认框，阻止 18px 以下小字或 18 字以上长句回流。
- `npm run build` 同时内置老人前台按钮真实可用审计，核心可见按钮必须产生真实反馈或状态变化；暂时无条件执行的按钮不会提前显示。
...
`npm run build` 当前等价于静态项目冒烟检查，会校验核心脚本、入口页面、后台页面、本机服务边界标记、老人前台可见文字审计和按钮真实可用审计。`npm run senior:audit` 可在已有本地服务器上单独复查普通老人模式是否出现小字或长句；`npm run senior:buttons` 可单独复查核心可见按钮是否都有真实反馈。
