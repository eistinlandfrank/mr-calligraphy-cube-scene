...
本次把前台“本机链接复制审计”从只展示复制记录和整份审计摘要，推进为每条链接复制回执都可本机重算 `receiptDigest` 的真实校验。用户如果手动改动 localStorage 里的链接 URL、目标类型、复制状态或标题，前台状态层和导出的审计 HTML 都会显示摘要不匹配。

完成内容：

- `recordLocalLinkCopyReceipt()` 新增持久化 `urlDigest` 和 `receiptDigest`。
- 新增 `verifyLocalLinkCopyReceiptDigest()`、`addLocalLinkCopyReceiptVerification()` 和 `createLocalLinkCopyReceiptDigestPayload()`。
- `getLocalLinkCopyAudit()` 返回 `verifiedCount`、`failedCount`、`legacyCount` 和 `latestReceipt`，每条回执新增 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 前台“本机链接复制审计”列表显示链接摘要、回执摘要和“本机校验通过 / 摘要不匹配 / 旧记录未校验”。
- 本机链接复制审计 HTML 新增“链接摘要”“回执摘要”“本机校验”和“重算摘要”。
- Playwright 前台练习流程验证本机分享链接、学习档案链接和站内报告链接复制回执为 `verified`，并临时篡改报告链接 URL 确认 `digest-mismatch` 被识别。

真实化说明：

- 数据来源：当前浏览器学习状态里的 `localLinkCopyReceipts`。
- 校验方式：按 `kind`、`targetType`、`targetId`、`title`、`urlDigest`、`copyStatus`、`copySucceeded` 和 `createdAt` 稳定 JSON + SHA-256 重算。
- 成功反馈：本机链接复制列表和 HTML 审计页显示“本机校验通过”。
- 失败反馈：URL 或声明字段被篡改后，读取 API 返回 `digest-mismatch` 和重算摘要。

仍待补：

- 这是本机摘要一致性校验，只能证明链接复制回执与自身 `receiptDigest` 声明字段一致；它不是剪贴板系统审计、公网访问日志、账号审计、跨设备分享统计、服务端签名、生产证书链或不可篡改证据链。
...
本次把前台“学习档案仓库导出回执审计”从只展示同步包导出记录、文件摘要、包摘要和回执摘要，推进为每条仓库导出回执都可本机重算 `receiptDigest` 的真实校验。用户导出学习档案 JSON 同步包后，如果手动改动 localStorage 里的文件名、包摘要、文件摘要、Workspace、档案数量或导出时间，前台状态层和导出的审计 HTML 都会显示摘要不匹配。

完成内容：

- 新增 `verifyHistoryRepositoryExportReceiptDigest()`、`addHistoryRepositoryExportReceiptVerification()` 和 `createHistoryRepositoryExportReceiptDigestPayload()`。
- `getHistoryRepositoryExportAudit()` 返回 `verifiedCount`、`failedCount` 和 `legacyCount`，每条回执新增 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 前台“学习档案仓库导出回执”列表显示“本机校验通过 / 摘要不匹配 / 旧记录未校验”。
- 学习档案仓库导出回执 HTML 审计页新增“本机校验”和“重算摘要”。
- Playwright 学习档案仓库流程验证真实 JSON 同步包导出回执为 `verified`，并临时篡改文件名确认 `digest-mismatch` 被识别。

真实化说明：

- 数据来源：当前浏览器学习状态里的 `historyRepositoryExportReceipts`。
- 校验方式：按 `kind`、`filename`、`byteLength`、`fileDigest`、`packageId`、`packageDigest`、`recordCount`、`practiceCount`、`artworkCount`、`reportCount`、`stageCount`、`workspaceId` 和 `exportedAt` 稳定 JSON + SHA-256 重算。
- 成功反馈：学习档案仓库导出回执列表和 HTML 审计页显示“本机校验通过”。
- 失败反馈：回执声明字段被篡改后，读取 API 返回 `digest-mismatch` 和重算摘要。

仍待补：

- 这是本机摘要一致性校验，只能证明学习档案仓库导出回执与自身 `receiptDigest` 声明字段一致；它不是操作系统保存完成证明、云端下载日志、账号审计、服务端签名、生产证书链或不可篡改证据链。
