...
结论：四个入口 HTML 的静态按钮和导航链接已经没有 `demo-content` 或缺失标记；动态场景热点仍按演示素材处理。下一步要审计的是“标为真实的控件是否足够真实”。
...
| P0 | 剩余 `demo-content` 动作治理 | 用户最容易觉得“按钮是假的” | 四个入口 HTML 静态控件已清零；后续治理动态场景热点 |
