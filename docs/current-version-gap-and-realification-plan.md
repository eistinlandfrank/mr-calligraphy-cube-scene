...
| 学习计划 | 可按本机状态生成、勾选、顺延、复盘、管理计划项，显示任务依赖图，生成下周期，检查浏览器通知权限，触发页面打开时的一次性本机通知，导出/导入 JSON 同步包，配置远端 API endpoint 并通过 `fetch` 检查、推送、拉取计划仓库，可导出离线 HTML 计划单；计划变更已进入自动同步队列，拉取远端时会检测本机待同步冲突 | 缺真正账号登录、后台托管仓库、远端提醒、教师端通知和可视化冲突解决 UI | 继续增加账号同步、后台计划仓库、教师端通知、远端提醒和冲突解决界面 |
...
3. 学习计划已有到期、提醒、顺延、复盘、依赖图、周期循环、离线导出、本机提醒权限边界、JSON 同步包、远端 API adapter、自动同步队列和冲突检测；真正账号系统、后台托管仓库、远端提醒、教师端通知和可视化冲突解决 UI 仍未接入。
...
- 冲突已有数据层检测，但还没有前端可视化解决界面；当前建议先手动推送本机计划或后续补“保留本机/采用远端/另存副本”交互。
