...
| 模块 | 当前可用内容 | 不完善点 | 真实化方向 |
| --- | --- | --- | --- |
| 主后台编辑 | 能编辑对象、图层、灯光、导入模型、保存布局；项目档案区已显示统一 `ProjectRepository` 状态 | 保存主要在 localStorage / IndexedDB | 继续把本机项目仓库升级为远端保存和协作接口 |
...
| 项目档案 | 可导出/导入 JSON，含 schema、迁移、模型哈希、选择恢复、恢复审计和统一项目仓库状态 | 三方合并、完整 JSON 树、远端资产服务仍弱 | 增加服务端 repository、多人合并策略和远端资产完整性校验 |
...
4. 后台本机发布、统一项目仓库状态和远端发布 API adapter 已可用，但它们仍是本机 adapter 或用户配置 endpoint，不是部署、审核、账号权限或 CDN 托管。
...
| P1 | 统一项目仓库状态 | 主后台和写实后台长期分叉，用户难判断草稿、发布和资产是否齐 | 第一版已完成：`ProjectRepository` 状态、`project-scene-repository-v1` 统一视图、主后台仓库状态面板和项目 Schema 检查 |
