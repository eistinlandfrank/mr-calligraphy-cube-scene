...
| 学习计划 | 可按本机状态生成、勾选、顺延、复盘、管理计划项，并可导出离线 HTML 计划单 | 缺周期循环、任务依赖图、远端提醒和跨设备同步 | 增加周期规则、依赖图、本机提醒 fallback 与远端计划 repository |
...
3. 学习计划已有到期、提醒、顺延、复盘和离线导出，但还缺周期循环、依赖图和跨设备提醒。
...
| 优先级 | 功能 | 原因 | 交付物 |
| --- | --- | --- | --- |
| P0 | 学习计划周期循环和依赖图 | 计划已有提醒、复盘和导出，但还不能表达长期周期与任务关系 | 周期规则、依赖视图、周期复盘测试 |
...
目前版本的基础已经比最早的静态页面强很多，但还没有达到“真实产品”的标准。下一阶段不应该继续堆新 Demo，而应该继续补真实闭环，尤其是学习计划周期规则、浏览器级自动化验收、报告 PDF/云端适配和后台远端发布边界。
