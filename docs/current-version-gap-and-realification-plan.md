...
| 学习计划 | 可按本机状态生成、勾选、顺延、复盘、管理计划项，显示任务依赖图，并可导出离线 HTML 计划单 | 缺周期循环、远端提醒和跨设备同步 | 增加周期规则、本机提醒 fallback 与远端计划 repository |
...
3. 学习计划已有到期、提醒、顺延、复盘、依赖图和离线导出，但还缺周期循环和跨设备提醒。
...
| 优先级 | 功能 | 原因 | 交付物 |
| --- | --- | --- | --- |
| P0 | 学习计划周期循环 | 计划已有提醒、复盘、依赖图和导出，但还不能表达长期循环训练 | 周期规则、周期复盘测试、本机提醒边界 |
