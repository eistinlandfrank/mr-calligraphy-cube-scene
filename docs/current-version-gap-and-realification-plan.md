...
| 学习计划 | 可按本机状态生成、勾选、顺延、复盘、管理计划项，显示任务依赖图，生成下周期，检查浏览器通知权限，触发页面打开时的一次性本机通知，导出/导入 JSON 同步包，并可导出离线 HTML 计划单 | 缺真正云端账号同步、远端提醒和教师端通知 | 继续增加远端 API adapter、跨设备自动同步和教师端通知 |
...
3. 学习计划已有到期、提醒、顺延、复盘、依赖图、周期循环、离线导出、本机提醒权限边界和 JSON 同步包；真正云端账号同步、远端提醒和教师端通知仍未接入。
...
| 优先级 | 功能 | 原因 | 交付物 |
| --- | --- | --- | --- |
| P0 | 远端计划 API adapter 和跨设备提醒 | 计划已有 JSON 同步包和 repository 合同，但还没有云端账号同步、教师端通知或后台推送 | 远端 API adapter、账号同步状态、跨设备提醒策略 |
...
目前版本的基础已经比最早的静态页面强很多，但还没有达到“真实产品”的标准。下一阶段不应该继续堆新 Demo，而应该继续补真实闭环，尤其是远端计划 repository、浏览器级自动化验收、报告 PDF/云端适配和后台远端发布边界。
