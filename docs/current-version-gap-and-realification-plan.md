...
| 学习档案 | 有筛选、趋势、详情、回收站、导出、直达链接、远端 API 推送/拉取、分页 `nextPageUrl` 自动追取、同 ID 冲突审计、远端冲突另存副本、API 合同和本机 mock 服务 | 还没有账号登录、托管档案仓库、生产级分页查询、服务端教师批注审计和长期归档 | 继续增加账号化 history repository、云端详情 URL、字段级合并和服务端审计 |
...
- 当前同 ID 差异会跳过并保存本机冲突审计，可将远端冲突记录另存为本机副本或忽略审计；后续需要账号化服务端版本、字段级 merge、服务端审计签名、服务端游标重试和空间隔离。
...
- 自动追取是前端 adapter 的分页消费能力，不等同账号化托管仓库；冲突审计是本机记录，不等同服务端不可篡改审计；仍缺服务端游标重试、字段级合并、服务端审计和长期归档。
...
- 当前支持审计、另存副本和忽略审计；还没有逐字段合并 UI、服务端审计签名或账号空间隔离。
