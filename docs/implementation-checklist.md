...
- [x] **S13-03：保持书写场景触发逻辑**
  验收：首页默认隐藏写实书写层，进入“书写训练 / 进入临摹”后显示宣纸和“永”字。
...
- [x] **S13-05：书写台使用桌面俯视近景**
  验收：进入“进入临摹”时默认 pitch 为桌面上方视角，白色宣纸不被桌面横梁遮挡。

完成记录（2026-06-17）：

- 已把基础书写场景白色宣纸上的“永”字恢复为旧版样张的可见墨迹效果。
- 已保留当前两级交互菜单和写实桌面默认隐藏逻辑。
- 已用浏览器截图验证进入“书写训练 / 进入临摹”后 `MR_WRITING_DESK_VISIBLE=true`，白色宣纸和“永”字可见。
