...
- 默认 SceneConfig 已按旧版主场景坐标恢复：地面 `y=-3.12`、前墙 `z=-8`、后墙 `z=8`、左右墙 `x=±8`、天花板 `y=5.05`。
- 前台已接入 1-10 空间交互流程，点击步骤按钮或 3D 热点会更新 `activeStepIndex`，并同步到前台、后台和预览。
- 黑色展示屏已显示“永”字讲解内容和空间扫描线，不再只是空物件。
- 场景增加 AR Anchors、VR Room、MR Hotspots 的空间感表现，包括地面锚点、热点浮动标签和步骤路径。
...
- [x] **S5-06：保存到 localStorage / IndexedDB**
  验收：后台保存后刷新页面配置不丢失。
...
- `app/src/scene-config.ts` 已定义 SceneConfig、SceneObject、SceneHotspot、AnimationConfig 和 SceneStep。
- 默认配置版本更新为 `2.1.0-legacy-layout`，用于恢复旧版主场景布局并迁移早期 `localStorage` 配置。
- SceneConfig 已包含 camera、environment、objects、hotspots、animations、steps、activeStepIndex 和 updatedAt。
- 后台属性面板修改对象名称、位置、旋转、缩放、颜色、透明度后会写入 `mr-calligraphy-scene-config-v2`。
- Playwright 已验证后台修改对象名称和位置后，前台 `/` 与预览 `/preview` 能读取同一份保存配置。
- “恢复默认”按钮会回到 `DEFAULT_SCENE_CONFIG`，包括旧版房间布局、41 个默认物件、10 个热点和 10 个交互步骤。
- `S5-04` 暂不勾选：当前 AnimationConfig 已有 id/name/type/character/loop/duration/strokes，但还没有文档要求的 `target` 字段。
...
- SceneCanvas 已调用 `navigator.xr.isSessionSupported` 检测 `immersive-ar` 与 `immersive-vr`。
- 支持 WebXR 时显示进入 MR/AR 或 VR 的按钮；不支持时显示屏幕 3D 降级状态。
- 当前已完成屏幕模式的 AR/VR 空间感表现，但还没有在真实头显设备上验收沉浸会话、控制器射线和 XR UI 适配，因此 `S6-03` 到 `S6-07` 暂不勾选。
