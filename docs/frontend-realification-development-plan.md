...
这一版已经不是早期纯静态 Demo。前台已有本机学习状态、书写画布、作品保存、学习档案、报告、PDF/HTML/WebM/JSON 导出、学习计划、计划提醒边界、远端计划 API adapter、服务端合同和本机 mock 服务；主后台和写实后台已有对象编辑、模型导入、保存历史、本机发布、回滚、项目档案、远端发布包预检、审核锁和资产清单。
...
1. 给计划 repository 和远端发布 adapter 继续补生产服务端实现，明确账号权限、服务端合并、资产签名和审计字段。
