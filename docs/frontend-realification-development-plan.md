...
| 学习档案 | 本机历史、详情路由、回收站、趋势、作品集、标签编辑、导出、远端 API 推送/拉取、`nextPageUrl` 分页自动追取、同 ID 冲突审计、远端冲突另存副本、API 合同和本机 mock 服务已有第一版 | 还没有账号登录、托管档案仓库、生产级分页查询、长期归档和服务端教师批注审计 | 做账号化 history repository、云端详情 URL、字段级合并和服务端审计 |
...
- 学习档案远端网络中断、分页返回、同 ID 差异冲突、`nextPageUrl` 自动追取分页和冲突审计已在后续补齐，后续还需字段级合并和服务端审计。
...
- 计划仓库推送包被服务端 422 结构拒绝、网络中断的浏览器级提示验收已补齐，后续还需超时重试和批量队列失败恢复。
- 学习档案远端网络中断、分页返回、同 ID 差异冲突、`nextPageUrl` 自动追取分页和本机冲突审计已在后续补齐，后续还需字段级合并。
...
- 计划项新增/删除、依赖链调整、周期规则和服务端版本的字段级合并审计。
- 学习档案远端网络中断、分页返回、同 ID 差异冲突、`nextPageUrl` 自动追取分页和冲突审计已补齐，后续还需字段级合并和服务端审计。
...
- 学习档案字段级合并、账号化托管仓库、服务端游标重试、服务端审计签名和跨设备长期归档。
...
- 学习档案逐字段合并 UI、服务端冲突审计签名、账号空间隔离和长期归档。
