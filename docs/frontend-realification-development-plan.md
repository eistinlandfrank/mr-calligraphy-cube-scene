...
| 模块 | 当前可用 | 不完善点 | 真实化方向 |
| --- | --- | --- | --- |
| 主后台编辑 | 对象、图层、灯光、基础物体、导入模型、保存快照可用；主后台项目档案区已显示统一 `ProjectRepository` 状态 | 保存范围主要是当前浏览器 | 继续把本机项目 repository 接到远端保存和协作接口 |
...
| 项目档案 | JSON 导出/导入、schema、迁移、字段恢复、模型哈希、恢复审计和统一项目仓库状态可用 | 三方合并、完整 JSON 树、远端资产校验仍弱 | 增加服务端 repository、多人冲突解决和远端资产完整性校验 |
...
2. 继续扩展主后台和写实后台统一项目仓库：字段迁移、完整 diff、远端保存和协作接口。
