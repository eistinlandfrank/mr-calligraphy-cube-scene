...
| 书写练习 | 鼠标/触控笔迹、撤销、清空、回放、保存和基础评分可用；本机 `ScoreService` 会记录评分来源、算法版本、最近证据摘要、累计评分次数和采样点；`local-heuristic-v2.0.0` 已保存第一版范字笔顺和压感采样证据 | 仍缺笔锋路径、硬件适配、教师标定和专业评分模型 | 继续扩展笔画顺序校验、笔锋分析、硬件压感校准和服务端评分来源 |
...
- 评分结果显示证据点、覆盖范围、重心、停顿、压感、范字笔顺和维度理由；本机 `ScoreService` 已记录并展示最近评分证据摘要和算法版本。
...
本次把基础评分从“只有分数和少量统计项”推进到带版本、范字和压感证据的本机评分包。新的 `local-heuristic-v2.0.0` 会在练习结果、学习状态、评分服务摘要和笔画分析详情里保留同一套证据，后续接专业模型时可以按算法版本区分来源。
...
- 笔顺库是第一版文本参考，还没有逐笔轨迹匹配、笔锋路径识别或错序判定。
