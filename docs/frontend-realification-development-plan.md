...
本次把前台复制链接类按钮从“只弹一条成功提示”推进到“可回看、可导出的本机复制回执”。报告链接、学习档案链接、作品集链接、本机分享链接和远端分享链接复制后，都会进入同一份本机审计列表。

完成内容：

- 新增 `LOCAL_LINK_COPY_AUDIT_KIND = "mr-calligraphy-local-link-copy-audit-v1"` 和本机复制审计边界说明。
- 状态层新增 `localLinkCopyReceipts`，随学习状态持久化到 localStorage。
- `MRAppState.recordLocalLinkCopyReceipt()` 记录链接类型、目标 ID、标题、URL、复制状态和时间。
- `MRAppState.getLocalLinkCopyAudit({ limit })` 返回目标类型统计、复制状态统计、回执列表和 `auditDigest`。
...
- 前台新增 `localLinkCopyAudit` 面板，展示最近 5 条复制回执。
- 复制本机分享链接、远端分享链接、站内报告链接、学习档案链接和作品集链接时都会记录回执。
- Smoke 首页标记检查新增本机链接复制审计节点。
- `learning-state-check.js` 验证报告链接、档案链接、剪贴板成功、地址栏降级、边界、摘要和 HTML。
- Playwright 前台真实练习用例验证本机分享链接、站内报告链接进入审计列表，并下载链接复制审计 HTML。

真实化说明：

- 数据来源：`mr-calligraphy-learning-state-v1.localLinkCopyReceipts`。
- 写入状态：每次复制或降级复制都会写入本机回执，并通过 `link-copy` 写入学习动作事件。
- 成功反馈：面板显示最近复制数量、链接标题、类型、状态和时间。
- 失败反馈：剪贴板不可用时记录为地址栏降级或手动复制，不显示虚假的剪贴板成功。
- 刷新后复现方式：回执随学习状态持久化，刷新后仍可查看和导出。

仍待补：

- 该审计是浏览器本机复制回执，不是公网访问统计、账号审计、跨设备分享统计、教师端课堂日志或服务端不可篡改证据链。
...
本次把“导出仓库”JSON 同步包纳入作品导出回执审计。此前作品仓库 JSON 能迁移和验真，但回执面板只记录作品集、评阅表和评阅汇总；现在用户下载 `mr-calligraphy-artwork-repository-*.json` 后，也能在同一“作品导出回执”里看到文件摘要和包摘要。

完成内容：

- `normalizeArtworkExportType()` 新增 `artwork-repository-json` 类型。
- `formatArtworkExportTypeLabel()` 新增“作品仓库 JSON”标签。
- `getArtworkExportMimeType()` 对 JSON 导出返回 `application/json;charset=utf-8`。
- `downloadArtworkRepository()` 成功下载 JSON 包后写入作品导出回执。
- 作品导出审计边界更新为覆盖作品仓库 JSON、作品集 HTML、课堂评阅表 HTML 和评阅汇总 HTML。
- `learning-state-check.js` 新增作品仓库 JSON 回执、MIME、包摘要和文件摘要断言。
- Playwright 作品仓库用例验证“导出仓库”进入作品导出回执，并在最终审计 HTML 中出现“作品仓库 JSON”。

真实化说明：

- 数据来源：`MRAppState.getArtworkRepositoryPackage()` 生成的真实作品仓库包。
- 写入状态：下载 JSON 包后写入 `mr-calligraphy-learning-state-v1.artworkExportReceipts`。
- 成功反馈：作品仓库回执列表显示“作品仓库 JSON”、作品数量、文件摘要、包摘要和回执摘要。
- 失败反馈：没有作品时原本的“还没有可导出的作品仓库包”继续生效，不写入空回执。
- 刷新后复现方式：回执随学习状态持久化，可从“导出回执”下载 HTML 审计页。

仍待补：

- 该回执只能证明当前浏览器生成并发起了作品仓库 JSON 下载请求，不代表系统保存成功、账号云端归档、课堂作品墙或不可篡改证据链。
...
本次把前台站内报告“导出同步包”补成可追踪的本机回执闭环。用户下载报告仓库 JSON 后，可以在同一面板看到包里包含多少报告、多少教师批注报告、多少验真摘要，并导出 HTML 审计页留档。

完成内容：

- 新增 `REPORT_REPOSITORY_EXPORT_AUDIT_KIND = "mr-calligraphy-report-repository-export-audit-v1"` 和报告仓库导出边界说明。
- 学习状态新增 `reportRepositoryExportReceipts`，随 `mr-calligraphy-learning-state-v1` 持久化。
- `MRAppState.recordReportRepositoryExportReceipt()` 记录文件名、MIME、字节数、报告数量、教师批注报告数量、验真数量、Workspace、包摘要、文件摘要和回执摘要。
- `MRAppState.getReportRepositoryExportAudit()` 返回 Workspace 统计、累计报告数量、最近回执列表和 64 位 `auditDigest`。
...
- `downloadReportRepository()` 成功发起 JSON 下载后写入报告仓库导出回执。
- 前台站内报告面板新增“同步包导出回执”列表和“导出回执”按钮。
- Smoke 页面标记检查新增 `reportRepositoryExportAudit`、`reportRepositoryExportAuditStatus`、`reportRepositoryExportAuditList` 和 `reportRepositoryExportAuditExport`。
- 控件清单更新为前台 `real-export 42`、`handled 117`、`missingHandler 0`。
- `learning-state-check.js` 验证同步包回执、报告数量、教师批注报告数量、验真数量、文件摘要、包摘要、边界和 HTML 审计页。
- Playwright 前台练习用例验证真实点击“导出同步包”、JSON 下载、回执面板、localStorage 持久化和 HTML 审计下载。

真实化说明：

- 数据来源：当前浏览器报告记录、报告验真摘要和 `MRAppState.getReportRepositoryPackage()` 生成的 `mr-calligraphy-report-repository-v1` 包。
- 写入状态：JSON 同步包下载请求成功发起后写入 `reportRepositoryExportReceipts`。
- 成功反馈：站内报告面板显示同步包文件名、报告数量、教师批注报告数量、验真数量、包摘要、文件摘要和回执摘要。
- 导出反馈：点击“导出回执”会下载报告仓库导出回执 HTML 审计页；无回执时按钮禁用。
- 刷新后复现方式：回执随学习状态持久化，可从同一面板重新导出。

仍待补：

- 当前只证明当前浏览器生成并发起了报告仓库 JSON 下载请求，并记录生成内容摘要；它不代表系统保存完成、云端报告仓库日志、账号审计、生产证书签章或不可篡改证据链。
...
本次把主后台“导出差异报告”补成可追踪的本机审计闭环。导入项目档案或拉取远端项目仓库版本后，用户导出的恢复前 HTML 差异报告会写入回执，保留来源、风险、恢复选择、文件摘要和预览摘要。

完成内容：

- 新增 `mr-calligraphy-project-impact-export-audit-v1` 本机审计包。
- `downloadImportImpactReport()` 下载 HTML 后自动写入差异报告导出回执。
- 新增 `recordProjectImpactExportReceipt()`、`getProjectImpactExportAudit()`、`getProjectImpactExportAuditExport()` 和 `downloadProjectImpactExportAudit()`。
...
- Playwright 主后台用例验证远端项目仓库版本拉取后真实点击“导出差异报告”、HTML 下载、回执面板、localStorage 持久化和 HTML 审计下载。

真实化说明：

- 数据来源：`createArchivePreview()` 生成的导入预览、当前恢复选择、风险摘要和远端项目仓库来源摘要。
...
本次把主后台“恢复审计”从只展示 `recordDigest`，推进为会主动重算摘要的本机验真。恢复记录如果被手动篡改，后台和导出的恢复审计 HTML 都会显示校验失败。

完成内容：

- `getRestoreAuditLog()` 读取恢复审计记录时会按原始写入字段重算 `recordDigest`。
- 恢复审计返回 `verifiedCount`、`failedCount`、`legacyCount`，每条记录新增 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 主后台“恢复审计”状态文案显示本机校验通过数量、失败数量和旧记录数量。
- 恢复审计列表每条记录显示“本机校验通过 / 本机校验失败 / 旧记录未校验”。
- 恢复审计 HTML 导出新增“本机校验”“重算摘要”和“校验说明”。
- Playwright 主后台用例验证正常恢复记录本机校验通过，并临时篡改恢复记录确认 `digest-mismatch` 能被识别。

真实化说明：

- 数据来源：`mr-calligraphy-project-archive-audit-v1` 中的恢复审计记录。
- 校验方式：去掉记录中的 `recordDigest`，还原生成摘要时使用的基础 ID，再用稳定 JSON + SHA-256 重算。
- 成功反馈：后台状态显示“本机校验通过 N 条”，导出 HTML 也保留同一重算摘要。
- 失败反馈：记录字段被篡改后，`getRestoreAuditLog()` 返回 `digest-mismatch`，并给出本机一致性校验失败说明。

仍待补：

- 这是当前浏览器本机一致性校验，只能证明恢复审计记录与自身摘要一致；它不是服务端签名、账号审批、远端不可篡改日志或多人审计链。
...
本次把“导出预览 JSON”从单次下载补成可追踪的本机回执闭环。用户在恢复前导出机器可读 JSON 后，后台会保留文件摘要、预览摘要、选择摘要、JSON 导出摘要和回执摘要，便于之后核对下载过哪份预览证据。

完成内容：

- 新增 `mr-calligraphy-project-import-preview-export-audit-v1` 本机审计包，保存最近 24 条导入预览 JSON 导出回执。
- `downloadImportPreviewJson()` 下载 JSON 后自动调用回执记录。
- 回执保留来源类型、远端/本机 packageId、Workspace、packageDigest、repositoryDigest、风险摘要、恢复选择数量、文件摘要、previewDigest、selectionDigest、exportDigest 和 receiptDigest。
- 主后台新增“预览 JSON 回执”审计面板。
...
- Smoke marker 和 Playwright 主后台用例均新增覆盖，确认回执写入、列表展示和 HTML 审计页下载。

真实化说明：

- 数据来源：当前 `createArchivePreview()` 结果、页面勾选的恢复方案和实际下载的 JSON 字符串。
- 执行动作：只记录 JSON 下载请求回执，不恢复、不覆盖项目状态。
- 成功反馈：主后台显示“已记录 N 条项目档案导入预览 JSON 导出回执”，列表展示来源、风险、选择数量、JSON 摘要和回执摘要。
- 失败反馈：没有导入预览时原导出按钮仍禁用；回执导出在无记录时禁用。

仍待补：

- 这是本机下载回执，不是服务端审批单、多人合并请求、账号签名、生产证书链或不可篡改审计。
...
本次把主后台“项目档案导出回执”从只显示摘要推进为会主动重算摘要的本机验真。用户如果手动修改 localStorage 里的回执字段，后台和导出的审计 HTML 都会显示摘要不匹配。

完成内容：

- `getProjectArchiveExportAudit()` 会为项目档案导出回执附加本机校验结果。
- 校验结果包含 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 主后台“导出回执”列表显示“本机校验通过 / 摘要不匹配 / 旧记录未校验”。
- 项目档案导出回执审计 HTML 保留同一校验状态和重算摘要。
- Playwright 主后台用例验证正常导出回执可通过本机校验，并篡改字段确认失败状态。

真实化说明：

- 数据来源：当前浏览器保存的 `mr-calligraphy-project-archive-export-audit-v1`。
- 执行动作：读取时重算 `receiptDigest`，不修改原始回执记录。
- 成功反馈：页面状态统计本机校验通过数量，列表显示校验通过。
- 失败反馈：摘要不一致时返回 `digest-mismatch`，HTML 审计也保留失败说明和重算摘要。
...
- 这是本机一致性校验，不是系统文件保存证明、云端备份证明、服务端签名、账号审批、生产证书链或不可篡改日志。
...
本次把主后台“差异报告导出回执”从只展示摘要推进为会主动重算摘要的本机验真。用户如果手动修改 localStorage 里的回执字段，后台和导出的审计 HTML 都会显示摘要不匹配。

完成内容：

- `getProjectImpactExportAudit()` 会为项目档案差异报告导出回执附加本机校验结果。
- 校验结果包含 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 主后台“差异报告导出回执”列表显示“本机校验通过 / 摘要不匹配 / 旧记录未校验”。
- 项目档案差异报告导出回执审计 HTML 保留同一校验状态和重算摘要。
- Playwright 主后台用例验证正常导出回执可通过本机校验，并篡改字段确认失败状态。

真实化说明：

- 数据来源：当前浏览器保存的 `mr-calligraphy-project-impact-export-audit-v1`。
- 执行动作：读取时重算 `receiptDigest`，不修改原始回执记录。
- 成功反馈：页面状态统计本机校验通过数量，列表显示校验通过。
- 失败反馈：摘要不一致时返回 `digest-mismatch`，HTML 审计也保留失败说明和重算摘要。
...
- 这是本机一致性校验，不是恢复动作证明、服务端签名、账号审批、生产证书链、多人合并审计或不可篡改日志。
...
本次把主后台“恢复审计导出回执”从只展示摘要推进为会主动重算摘要的本机验真。用户如果手动修改 localStorage 里的回执字段，后台和导出的审计 HTML 都会显示摘要不匹配。

完成内容：

- `getProjectRestoreAuditExportAudit()` 会为项目档案恢复审计导出回执附加本机校验结果。
- 校验结果包含 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 主后台“恢复审计导出回执”列表显示“本机校验通过 / 摘要不匹配 / 旧记录未校验”。
- 项目档案恢复审计导出回执审计 HTML 保留同一校验状态和重算摘要。
- Playwright 主后台用例验证正常导出回执可通过本机校验，并篡改字段确认失败状态。

真实化说明：

- 数据来源：当前浏览器保存的 `mr-calligraphy-project-restore-audit-export-v1`。
- 执行动作：读取时重算 `receiptDigest`，不修改原始回执记录。
- 成功反馈：页面状态统计本机校验通过数量，列表显示校验通过。
- 失败反馈：摘要不一致时返回 `digest-mismatch`，HTML 审计也保留失败说明和重算摘要。
...
- 这是本机一致性校验，不是系统文件保存证明、恢复动作审批、服务端签名、生产证书链、多人合并审计或不可篡改日志。
...
本次把前台“报告导出回执”从只展示摘要推进为会主动重算摘要的本机验真。用户如果手动修改 localStorage 里的报告导出回执字段，前台和导出的审计 HTML 都会显示摘要不匹配。

完成内容：

- `getReportExportAudit()` 会为报告 HTML/PDF 导出回执附加本机校验结果。
- 校验结果包含 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 前台“报告导出回执”列表显示“本机校验通过 / 摘要不匹配 / 旧记录未校验”。
- 报告导出回执审计 HTML 保留同一校验状态和重算摘要。
- Playwright 前台报告流程验证正常导出回执可通过本机校验，并篡改字段确认失败状态。

真实化说明：

- 数据来源：当前浏览器保存的 `reportExportReceipts`。
- 执行动作：读取时重算 `receiptDigest`，不修改原始回执记录。
- 成功反馈：页面状态统计本机校验通过数量，列表显示校验通过。
- 失败反馈：摘要不一致时返回 `digest-mismatch`，HTML 审计也保留失败说明和重算摘要。
...
- 这是本机一致性校验，不是操作系统保存完成证明、云端 PDF 渲染日志、账号下载审计、服务端签名、生产证书链或不可篡改日志。
...
本次把前台“报告对比导出回执”从只展示摘要推进为会主动重算摘要的本机验真。用户如果手动修改 localStorage 里的报告对比导出回执字段，前台和导出的审计 HTML 都会显示摘要不匹配。

完成内容：

- `getReportComparisonExportAudit()` 会为报告对比 HTML 导出回执附加本机校验结果。
- 校验结果包含 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 前台“报告对比导出回执”列表显示“本机校验通过 / 摘要不匹配 / 旧记录未校验”。
- 报告对比导出回执审计 HTML 保留同一校验状态和重算摘要。
- Playwright 前台报告流程验证正常对比导出回执可通过本机校验，并篡改字段确认失败状态。

真实化说明：

- 数据来源：当前浏览器保存的 `reportComparisonExportReceipts`。
- 执行动作：读取时重算 `receiptDigest`，不修改原始回执记录。
- 成功反馈：页面状态统计本机校验通过数量，列表显示校验通过。
- 失败反馈：摘要不一致时返回 `digest-mismatch`，HTML 审计也保留失败说明和重算摘要。
...
- 这是本机一致性校验，不是云端长期报告、跨设备下载日志、账号审计、服务端签名、生产证书链或不可篡改日志。
...
本次把前台“报告打印回执”从只展示请求记录升级为可核验的本机回执。点击“打印 / 保存 PDF”后，回执会继续记录浏览器打印请求；读取审计时会按声明字段重算 `receiptDigest`，正常记录显示本机校验通过，篡改记录显示摘要不匹配。

完成内容：

- `getReportPrintAudit()` 会为报告打印回执附加本机校验结果。
- 校验结果包含 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 前台“报告打印回执”列表显示“本机校验通过 / 摘要不匹配 / 旧记录未校验”。
- 报告打印回执审计 HTML 保留同一校验状态和重算摘要。
- Playwright 前台报告流程验证正常打印回执可通过本机校验，并篡改字段确认失败状态。

真实化说明：

- 数据来源：当前浏览器保存的 `reportPrintReceipts`。
- 执行动作：读取时重算 `receiptDigest`，不修改原始回执记录。
- 成功反馈：页面状态统计本机校验通过数量，列表显示校验通过。
- 失败反馈：摘要不一致时返回 `digest-mismatch`，HTML 审计也保留失败说明和重算摘要。
...
- 这是本机一致性校验，不是操作系统打印完成证明、云端 PDF 渲染日志、账号打印审计、服务端签名、生产证书链或不可篡改日志。
...
本次把前台“计划导出回执”从只展示请求记录升级为可核验的本机回执。下载学习计划 HTML 或日历 ICS 后，回执会继续记录文件摘要、任务数量、完成数量和事件数量；读取审计时会按声明字段重算 `receiptDigest`，正常记录显示本机校验通过，篡改记录显示摘要不匹配。

完成内容：

- `getPlanExportAudit()` 会为计划 HTML/ICS 导出回执附加本机校验结果。
- 校验结果包含 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 前台“计划导出回执”列表显示“本机校验通过 / 摘要不匹配 / 旧记录未校验”。
- 计划导出回执审计 HTML 保留同一校验状态和重算摘要。
- Playwright 前台流程验证正常导出回执可通过本机校验，并篡改字段确认失败状态。

真实化说明：

- 数据来源：当前浏览器保存的 `planExportReceipts`。
- 执行动作：读取时重算 `receiptDigest`，不修改原始回执记录。
- 成功反馈：页面状态统计本机校验通过数量，列表显示校验通过。
- 失败反馈：摘要不一致时返回 `digest-mismatch`，HTML 审计也保留失败说明和重算摘要。
...
- 这是本机一致性校验，不是操作系统保存完成证明、云端下载日志、账号审计、服务端签名、生产证书链或不可篡改日志。
...
本次把前台“计划仓库导出回执”从只展示同步包记录升级为可核验的本机回执。下载计划仓库 JSON 同步包后，回执会继续记录 Workspace、包摘要、文件摘要、计划数量和字节数；读取审计时会按声明字段重算 `receiptDigest`，正常记录显示本机校验通过，篡改记录显示摘要不匹配。

完成内容：

- `getPlanRepositoryExportAudit()` 会为计划仓库 JSON 同步包导出回执附加本机校验结果。
- 校验结果包含 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 前台“计划仓库导出回执”列表显示“本机校验通过 / 摘要不匹配 / 旧记录未校验”。
- 计划仓库导出回执审计 HTML 保留同一校验状态和重算摘要。
- Playwright 计划仓库流程验证正常导出回执可通过本机校验，并篡改字段确认失败状态。

真实化说明：

- 数据来源：当前浏览器保存的 `planRepositoryExportReceipts`。
- 执行动作：读取时重算 `receiptDigest`，不修改原始回执记录。
- 成功反馈：页面状态统计本机校验通过数量，列表显示校验通过。
- 失败反馈：摘要不一致时返回 `digest-mismatch`，HTML 审计也保留失败说明和重算摘要。
...
- 这是本机一致性校验，不是云端仓库写入证明、跨设备下载日志、账号审计、服务端签名、生产证书链或不可篡改日志。
...
本次把前台“复盘导出回执”从只展示导出记录升级为可核验的本机回执。下载作品图片、复盘证据 HTML、作品分享页 HTML 或学习报告 HTML 后，回执会继续记录文件摘要、来源、MIME、字节数和导出时间；读取审计时会按声明字段重算 `receiptDigest`，正常记录显示本机校验通过，篡改记录显示摘要不匹配。

完成内容：

- `getReviewExportAudit()` 会为四类复盘导出回执附加本机校验结果。
- 校验结果包含 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 前台“复盘导出回执”列表显示“本机校验通过 / 摘要不匹配 / 旧记录未校验”。
- 复盘导出回执审计 HTML 保留同一校验状态和重算摘要。
- Playwright 前台练习流程验证正常复盘导出回执可通过本机校验，并篡改字段确认失败状态。

真实化说明：

- 数据来源：当前浏览器保存的 `reviewExportReceipts`。
- 执行动作：读取时重算 `receiptDigest`，不修改原始回执记录。
- 成功反馈：页面状态统计本机校验通过数量，列表显示校验通过。
- 失败反馈：摘要不一致时返回 `digest-mismatch`，HTML 审计也保留失败说明和重算摘要。
...
- 这是本机一致性校验，不是操作系统下载完成证明、云端下载日志、账号审计、服务端签名、生产证书链或不可篡改日志。
...
本次把前台“学习档案详情操作回执”从只展示操作记录升级为可核验的本机回执。下载详情图片、下载报告 HTML 或复制详情直达链接后，回执会继续记录目标记录、文件或链接摘要、复制状态和记录时间；读取审计时会按声明字段重算 `receiptDigest`，正常记录显示本机校验通过，篡改记录显示摘要不匹配。

完成内容：

- `getHistoryDetailActionAudit()` 会为图片下载、报告下载和链接复制回执附加本机校验结果。
- 校验结果包含 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 前台“详情操作回执”列表显示“本机校验通过 / 摘要不匹配 / 旧记录未校验”。
- 学习档案详情操作回执审计 HTML 保留同一校验状态和重算摘要。
- Playwright 前台练习流程验证正常详情操作回执可通过本机校验，并篡改字段确认失败状态。

真实化说明：

- 数据来源：当前浏览器保存的 `historyDetailActionReceipts`。
- 执行动作：读取时重算 `receiptDigest`，不修改原始回执记录。
- 成功反馈：页面状态统计本机校验通过数量，列表显示校验通过。
- 失败反馈：摘要不一致时返回 `digest-mismatch`，HTML 审计也保留失败说明和重算摘要。
...
- 这是本机一致性校验，不是操作系统下载完成证明、剪贴板系统审计、云端访问日志、账号审计、服务端签名、生产证书链或不可篡改日志。
...
本次把前台“作品导出回执审计”从只展示导出记录升级为可核验的本机回执。导出作品仓库 JSON、作品集 HTML、课堂评阅表 HTML 或评阅汇总 HTML 后，回执会继续记录导出类型、文件名、MIME、字节数、文件摘要、包摘要、作品数量和导出时间；读取审计时会按声明字段重算 `receiptDigest`，正常记录显示本机校验通过，篡改记录显示摘要不匹配。

完成内容：

- `getArtworkExportAudit()` 会为四类作品导出回执附加本机校验结果。
- 校验结果包含 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 前台“作品导出回执”列表显示“本机校验通过 / 摘要不匹配 / 旧记录未校验”。
- 作品导出回执审计 HTML 保留同一校验状态和重算摘要。
- Playwright 作品仓库流程验证正常作品导出回执可通过本机校验，并篡改字段确认失败状态。

真实化说明：

- 数据来源：当前浏览器保存的 `artworkExportReceipts`。
- 执行动作：读取时重算 `receiptDigest`，不修改原始回执记录。
- 成功反馈：页面状态统计本机校验通过数量，列表显示校验通过。
- 失败反馈：摘要不一致时返回 `digest-mismatch`，HTML 审计也保留失败说明和重算摘要。
...
## 164. 2026-06-13 新增学习档案仓库导出回执本机校验

本次把前台“学习档案仓库导出回执审计”从只展示导出记录升级为可核验的本机回执。导出学习档案 JSON 同步包后，回执会继续记录文件名、MIME、字节数、文件摘要、包 ID、包摘要、Workspace、练习/作品/报告/阶段数量和导出时间；读取审计时会按声明字段重算 `receiptDigest`，正常记录显示本机校验通过，篡改记录显示摘要不匹配。

完成内容：

- `getHistoryRepositoryExportAudit()` 会为学习档案仓库导出回执附加本机校验结果。
- 校验结果包含 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 前台“学习档案仓库导出回执”列表显示“本机校验通过 / 摘要不匹配 / 旧记录未校验”。
- 学习档案仓库导出回执审计 HTML 保留同一校验状态和重算摘要。
- Playwright 学习档案仓库流程验证正常同步包导出回执可通过本机校验，并篡改字段确认失败状态。

真实化说明：

- 数据来源：当前浏览器保存的 `historyRepositoryExportReceipts`。
- 执行动作：读取时重算 `receiptDigest`，不修改原始回执记录。
- 成功反馈：页面状态统计本机校验通过数量，列表显示校验通过。
- 失败反馈：摘要不一致时返回 `digest-mismatch`，HTML 审计也保留失败说明和重算摘要。
...
- 中文 commit message：`新增学习档案导出回执校验`
