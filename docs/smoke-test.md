...
- 学习状态检查：模拟同字两幅作品，确认基础评分服务、本机讲解服务、本机学习动作审计、`MRAppState.getArtworkComparison()` 会生成前后作品、评分差、笔画差、采样差、截图和维度差，并验证作品集搜索、标签筛选、标签编辑、作品导出回执审计和本机校验、localStorage 持久化、学习档案同步仓库、学习档案仓库导出回执审计和本机校验、学习档案仓库 Workspace 请求头和包字段、学习档案仓库 mock 服务按空间隔离、学习档案仓库教师批注同步摘要、学习档案仓库回执审计导出和回执本机校验、学习档案冲突审计和字段级合并、作品分享页 HTML、本机分享链接服务、本机链接复制审计和本机校验、学习档案详情操作回执审计和本机校验、远端分享 API adapter、分享 API Workspace 请求头和包字段、分享 mock 服务按空间隔离、分享远端撤销、回执审计导出和回执本机校验、书写视频导出记录、PNG 封面、本机队列和失败重试、复盘导出回执审计和本机校验、报告原生 PDF、报告 PDF 能力雷达图、报告 PDF 分数趋势图、报告 PDF 作品截图嵌入、报告本机验真摘要、报告导出回执审计、报告对比导出回执审计、报告打印回执审计、报告仓库本机 JSON 同步包、报告仓库远端 API adapter、报告仓库 Workspace 请求头和包字段、报告仓库签名回执审计导出、报告仓库 mock 服务按空间隔离、报告仓库冲突审计、报告冲突字段级合并和远端副本另存、报告教师批注、报告对比离线 HTML、评分证据、学习阶段记录、任务依赖与完成规则、学习计划提醒、顺延、复盘状态、学习计划提醒服务边界、计划提醒回执审计和本机校验、学习计划日历提醒导出、计划导出回执审计和本机校验、学习计划同步仓库、远端计划 API adapter、计划仓库 Workspace 请求头和包字段、计划仓库导出回执审计和本机校验、计划仓库回执审计导出、计划仓库回执本机校验、计划仓库 mock 服务按空间隔离、学习计划自动同步队列、计划同步冲突检测、计划冲突另存副本、学习计划依赖图、学习计划周期循环和学习计划离线 HTML 导出。
- 学习路径动作覆盖检查会防止 `SCENES` 中新增动作时漏掉真实状态标记或处理分支；当前覆盖 10 个场景、30 个动作、30 个状态标记和 30 个处理分支。
- 前台可见菜单已经收敛为 3 个一级方向和二级动作；10 个 `SCENES` 继续作为内部路由和真实动作覆盖对象，不再作为外显菜单。
- 页面可访问检查：覆盖 `/`、`/main-admin.html`、`/realistic-demo.html`、`/realistic-admin.html`，并确认页面包含关键 DOM / script 标记，包括前台服务边界面板、学习动作审计面板、学习步骤路由、学习热点路由、模型展示路由、基础评分服务摘要、AI 讲解本机语音状态、本机讲解服务摘要、学习档案重命名表单、作品标签编辑表单、作品导出回执审计入口、学习档案详情操作回执入口、学习档案同步仓库状态/导入/导出入口、远端学习档案 API endpoint/token/Workspace/检查/推送/拉取入口、学习档案仓库回执审计入口、学习计划提醒摘要、学习计划本机提醒服务状态/权限入口、计划提醒回执审计入口、学习计划同步仓库状态/导入/导出入口、远端计划 API endpoint/token/Workspace/检查/推送/拉取入口、计划仓库回执审计入口、计划同步冲突面板和保留本机/采用远端/另存副本入口、学习计划周期摘要、学习计划依赖图、学习计划项表单编辑入口、学习计划导出入口、学习计划提醒日历导出入口、计划导出回执审计入口、学习计划下周期入口、作品分享远端 API endpoint/token/Workspace/检查/发布/撤销入口、作品分享远端回执审计入口、复盘导出回执审计入口、报告本机验真摘要、报告仓库状态、报告仓库本机同步包导入/导出入口、远端报告 API endpoint/token/Workspace/检查/推送/拉取入口、报告仓库签名回执审计入口、报告仓库冲突审计入口、报告原生 PDF 下载入口、报告导出回执审计入口、报告对比导出回执审计入口、报告打印回执审计入口、报告本机教师批注入口、报告对比、报告对比导出、多报告趋势、字段多选控件、字段分组模板、趋势悬浮提示入口、提示固定/复制入口、趋势缩放入口、逐点明细入口、主后台服务边界面板、主后台基础物体新增/更新控件、主后台导入模型贴图替换入口、主后台导入模型历史文件清理入口、主后台项目档案导出回执审计入口、主后台项目仓库状态、主后台远端项目仓库 API endpoint/token/Workspace 控件、主后台远端项目仓库版本选择和拉取预览入口、主后台项目仓库回执审计与本机校验入口、主后台项目档案差异报告入口、主后台项目档案恢复审计入口、主后台和写实后台远端发布 API endpoint/token/Workspace 控件、远端发布审核/发布锁控件、远端发布回执审计控件、写实后台服务边界面板、写实后台对象位置/删除/恢复/撤回控件、写实导入模型贴图替换入口、写实导入模型删除审计清理入口、写实样张相机控件以及两个后台的本机权限风险提示；前台首页不再要求独立写实样张入口，写实桌面层改由书写台状态切换验证。
...
本轮新增项目仓库包导出回执本机校验验收：`getProjectRepositoryExportAudit()` 会按回执声明字段重算 `receiptDigest`，主后台“仓库包导出回执”显示本机校验通过；Playwright 会验证正常回执为 `verified`，并临时篡改回执确认 `digest-mismatch` 能被识别。
...
本轮新增项目档案差异报告回执本机校验验收：`getProjectImpactExportAudit()` 会按回执声明字段重算 `receiptDigest`，主后台“差异报告导出回执”显示本机校验通过；Playwright 会验证正常回执为 `verified`，并临时篡改回执确认 `digest-mismatch` 能被识别。
...
本轮新增项目档案恢复审计导出回执本机校验验收：`getProjectRestoreAuditExportAudit()` 会按回执声明字段重算 `receiptDigest`，主后台“恢复审计导出回执”显示本机校验通过；Playwright 会验证正常回执为 `verified`，并临时篡改回执确认 `digest-mismatch` 能被识别。
本轮新增项目档案恢复审计本机校验验收：`getRestoreAuditLog()` 会按恢复审计声明字段重算 `recordDigest`，主后台“恢复审计”显示本机校验通过数量；Playwright 会验证正常恢复记录为 `verified`，并临时篡改恢复记录确认 `digest-mismatch` 能被识别。
本轮新增项目仓库包文件导入预览验收：主后台“导入项目档案”入口可识别本机 `mr-calligraphy-project-repository-package-v1` JSON 包；Playwright 会验证导出的仓库包能重新进入“本机项目仓库包预览”，并验证篡改但未重签的仓库包被 `packageDigest` 校验拒绝。
本轮新增项目档案导入预览 JSON 导出验收：主后台恢复预览区新增 `projectImportExportPreviewJson`；Playwright 会在本机项目仓库包预览后下载 `mr-calligraphy-import-preview-*.json`，验证 kind、来源类型、packageId、Workspace、包摘要、恢复选择、previewDigest、selectionDigest 和 exportDigest。
本轮新增项目档案导入预览 JSON 导出回执验收：主后台新增 `projectImportPreviewExportAudit`、`projectImportPreviewExportAuditStatus`、`projectImportPreviewExportAuditList` 和 `projectImportPreviewExportAuditExport`；Playwright 会在下载导入预览 JSON 后验证 `mr-calligraphy-project-import-preview-export-audit-v1` 写入文件摘要、previewDigest、selectionDigest、exportDigest、receiptDigest、来源 packageId 和 Workspace，并下载 HTML 回执审计页。
本轮新增项目档案导入预览 JSON 回执本机校验验收：`getProjectImportPreviewExportAudit()` 会按回执声明字段重算 `receiptDigest`，主后台“预览 JSON 回执”显示本机校验通过；Playwright 会验证正常回执为 `verified`，并临时篡改回执确认 `digest-mismatch` 能被识别。
...
本轮新增项目档案导出回执本机校验验收：`getProjectArchiveExportAudit()` 会按回执声明字段重算 `receiptDigest`，主后台“导出回执”显示本机校验通过；Playwright 会验证正常回执为 `verified`，并临时篡改回执确认 `digest-mismatch` 能被识别。
...
本轮新增本机链接复制回执本机校验验收：`recordLocalLinkCopyReceipt()` 会持久化 `urlDigest` 和 `receiptDigest`；`getLocalLinkCopyAudit()` 会按链接复制回执声明字段重算摘要；前台“本机链接复制审计”和 HTML 审计页会显示“本机校验通过”，Playwright 会临时篡改报告链接 URL 确认 `digest-mismatch` 能被识别。
...
本轮新增报告打印回执本机校验验收：`getReportPrintAudit()` 会按报告打印回执声明字段重算 `receiptDigest`，前台“报告打印回执”显示本机校验通过；Playwright 会验证正常打印回执为 `verified`，并临时篡改回执确认 `digest-mismatch` 能被识别。
...
本轮新增报告导出回执本机校验验收：`getReportExportAudit()` 会按报告导出回执声明字段重算 `receiptDigest`，前台“报告导出回执”显示本机校验通过；Playwright 会验证正常 HTML/PDF 回执为 `verified`，并临时篡改回执确认 `digest-mismatch` 能被识别。
...
本轮新增报告对比导出回执本机校验验收：`getReportComparisonExportAudit()` 会按报告对比导出回执声明字段重算 `receiptDigest`，前台“报告对比导出回执”显示本机校验通过；Playwright 会验证正常回执为 `verified`，并临时篡改回执确认 `digest-mismatch` 能被识别。
...
本轮新增作品导出回执本机校验验收：`getArtworkExportAudit()` 会按作品导出回执声明字段重算 `receiptDigest`，前台“作品导出回执”显示本机校验通过；Playwright 会验证四类导出回执均为 `verified`，并临时篡改评阅汇总文件名确认 `digest-mismatch` 能被识别。
...
本轮新增学习档案仓库导出回执本机校验验收：`getHistoryRepositoryExportAudit()` 会按学习档案仓库导出回执声明字段重算 `receiptDigest`，前台“学习档案仓库导出回执”显示本机校验通过；Playwright 会验证正常 JSON 同步包导出回执为 `verified`，并临时篡改文件名确认 `digest-mismatch` 能被识别。
...
本轮新增计划提醒回执本机校验验收：`getPlanReminderAudit()` 会按计划提醒回执声明字段重算 `receiptDigest`，前台“计划提醒回执”显示本机校验通过；Playwright 会验证正常提醒回执为 `verified`，并临时篡改回执确认 `digest-mismatch` 能被识别。
...
本轮新增计划导出回执本机校验验收：`getPlanExportAudit()` 会按计划导出回执声明字段重算 `receiptDigest`，前台“计划导出回执”显示本机校验通过；Playwright 会验证正常 HTML/ICS 回执为 `verified`，并临时篡改回执确认 `digest-mismatch` 能被识别。
...
本轮新增计划仓库导出回执本机校验验收：`getPlanRepositoryExportAudit()` 会按计划仓库导出回执声明字段重算 `receiptDigest`，前台“计划仓库导出回执”显示本机校验通过；Playwright 会验证正常 JSON 同步包导出回执为 `verified`，并临时篡改回执确认 `digest-mismatch` 能被识别。
...
本轮新增复盘导出回执本机校验验收：`getReviewExportAudit()` 会按复盘导出回执声明字段重算 `receiptDigest`，前台“复盘导出回执”显示本机校验通过；Playwright 会验证正常四类复盘导出回执为 `verified`，并临时篡改回执确认 `digest-mismatch` 能被识别。
...
本轮新增学习档案详情操作回执本机校验验收：`getHistoryDetailActionAudit()` 会按详情操作回执声明字段重算 `receiptDigest`，前台“详情操作回执”显示本机校验通过；Playwright 会验证正常图片下载/链接复制回执为 `verified`，并临时篡改链接回执确认 `digest-mismatch` 能被识别。
...
- Playwright 已覆盖首批真实交互闭环、前台学习详情总结、前台服务边界状态、前台学习动作审计导出、复盘导出回执审计导出、学习档案详情操作回执审计导出和本机校验、计划提醒回执审计导出、计划导出回执审计导出、计划仓库导出回执审计导出、学习档案仓库导出回执审计导出和本机校验、报告仓库导出回执审计导出、项目档案导出回执审计导出、学习档案批量回执审计导出、本机链接复制审计导出和本机校验、报告打印回执审计导出、后台服务边界状态、本机后台操作者审计、本机后台角色权限门控、核心入口移动端视口验收、作品仓库本机导入导出、包摘要验真和冲突审计、作品集离线 HTML 导出、课堂评阅表 HTML 导出、课堂评阅 JSON 导入回写和摘要验真、课堂评阅汇总 HTML 导出、作品导出回执审计导出和本机校验、远端分享 API adapter、作品分享远端失败恢复、作品分享远端撤销和回执审计导出、书写视频 WebM/PNG 封面导出、本机队列和失败重试、报告教师批注角色与本机签名摘要、学习档案远端同步、主后台项目仓库远端版本恢复风险预览、主后台远端发布回执与本机校验、主后台导入模型主色调/透明度/PBR/发布差异明细/文件替换/贴图替换/孤立贴图清理、主后台导入模型删除审计和历史文件本机清理、写实导入模型主色调/透明度/PBR/发布差异明细/文件替换/贴图替换/孤立贴图清理、写实导入模型软删除审计、写实导入模型已删除文件本机清理和写实发布历史源码；测试仍未覆盖所有下载、服务端资产回收、不可篡改服务端审计和完整移动设备矩阵。
