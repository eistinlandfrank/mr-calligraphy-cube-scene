...
本轮新增作品导出回执审计验收：前台新增 `artworkExportAudit`、`artworkExportAuditStatus`、`artworkExportAuditList` 和 `artworkExportAuditExport`；状态层会生成 `mr-calligraphy-artwork-export-audit-v1`、作品集/课堂评阅表/评阅汇总导出回执、作品数量、评阅数量、文件摘要、包摘要和 64 位 `auditDigest`；Playwright 作品仓库用例会验证三类真实下载、回执持久化和 HTML 审计页下载。
