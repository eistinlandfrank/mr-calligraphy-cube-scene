...
- 主后台导入真实 `.glb` 模型后更新主色调，确认 `mr-calligraphy-main-scene-layout-v1.importedModels[*].color` 写入草稿，发布后进入 `mr-calligraphy-main-scene-published-v1`，普通前台发布布局也读取该颜色。
...
- 写实后台导入真实 `.glb` 模型后更新主色调，确认 `mr-calligraphy-realistic-layout-v1.importedModels[*].color` 写入草稿，发布后进入 `mr-calligraphy-realistic-published-v1`，写实演示页发布布局也读取该颜色。
...
- Playwright 已覆盖首批真实交互闭环、远端分享 API adapter、作品分享远端撤销和回执审计导出、书写视频 WebM/PNG 封面导出、本机队列和失败重试、报告教师批注持久化、学习档案远端同步、主后台远端发布回执、主后台导入模型主色调编辑、主后台导入模型删除审计、写实导入模型主色调编辑、写实导入模型软删除审计和写实发布历史源码；测试仍未覆盖所有下载、导入模型贴图/透明度/PBR 编辑、回收站和移动端视口。
