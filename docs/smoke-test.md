...
本轮新增静态控件处理器覆盖验收：`node scripts/control-inventory.js --check` 会按入口页面扫描实际加载脚本，要求所有真实状态按钮和导出按钮都有 `click`、`submit`、批量 selector 或初始化参数绑定；当前前台 107 个、主后台 53 个、写实演示 3 个、写实后台 34 个真实控件均为 `missingHandler 0`。
