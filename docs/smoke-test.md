...
- Playwright 已覆盖首批真实交互闭环、报告教师批注持久化、主后台远端发布回执和写实发布历史源码，但当前环境缺少依赖时仍需在具备 npm 代理认证的环境执行；测试仍未覆盖所有下载、导入模型、回收站和移动端视口。
