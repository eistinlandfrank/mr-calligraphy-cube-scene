...
- 前台作品仓库用例会从作品集 UI 点击“导出仓库”下载 `mr-calligraphy-artwork-repository-*.json`，确认包内包含作品、关联练习、评分证据、截图、摘要和边界说明；随后清空本机学习状态，通过“导入仓库”文件选择器恢复作品和关联练习，确认 `artworkRepository` 状态、作品卡片和 localStorage 都真实更新；再次导入同 ID 差异包时会显示作品仓库冲突审计，点击“另存导入副本”后作品集新增副本且原作品不被覆盖。
