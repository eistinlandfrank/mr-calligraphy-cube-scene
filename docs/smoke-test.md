...
- 前台主房间、主后台和写实后台会采样 WebGL canvas 像素，确认画布不是空白 DOM；前台手机视口还会检查服务边界面板显示本机真实、远端 Adapter 和生产云端未接入状态。
