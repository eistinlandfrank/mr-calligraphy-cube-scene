...
- 站内报告配置远端 endpoint/token 后，用浏览器路由模拟报告仓库 API，覆盖检查远端、推送报告包、Bearer token、远端 packageId 和签名回执持久化、拉取远端包、教师批注和本机验真摘要随包同步，并验证同 ID 差异报告会出现冲突审计且可按字段合并。
