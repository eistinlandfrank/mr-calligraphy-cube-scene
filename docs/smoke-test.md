...
- 前台学习档案配置远端 endpoint/token 后，用浏览器路由模拟学习档案仓库，覆盖检查远端、推送档案包、Bearer token、远端 packageId 持久化、拉取远端包、分页第二页自动追取、冲突审计面板、远端冲突另存副本和 `historyRepository` 状态更新。
