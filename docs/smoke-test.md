...
- Playwright 已覆盖首批真实交互闭环、远端分享 API adapter、作品分享远端撤销和回执审计导出、书写视频 WebM/PNG 封面导出、本机队列和失败重试、报告教师批注持久化、学习档案远端同步、主后台远端发布回执、主后台导入模型主色调编辑、主后台导入模型删除审计、写实导入模型软删除审计和写实发布历史源码；测试仍未覆盖所有下载、导入模型贴图/透明度/PBR 编辑、回收站和移动端视口。
