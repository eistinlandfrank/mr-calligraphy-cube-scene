...
- 远端发布检查：模拟主后台和写实后台本机发布版本，确认 `MRProjectRemotePublish` 会拒绝未配置/非法 endpoint，保存 HTTP endpoint/token，生成带 manifest 和 SHA-256 摘要的发布包，执行发布包本机预检，携带 Bearer header，GET 检查远端，POST 当前发布包，并持久化 packageId、releaseId、packageDigest 和远端状态。
...
该命令会模拟主后台和写实后台当前本机发布版本，验证远端发布 adapter 能生成 `mr-calligraphy-remote-publish-package-v1` 发布包和 `mr-calligraphy-remote-publish-manifest-v1` manifest，正常包预检通过，篡改包预检失败，拒绝未配置或非法 endpoint，保存 HTTP endpoint 和 token，真实调用 mock `fetch` 做 GET 检查和 POST 推送，并确认 Authorization header、packageId、releaseId、packageDigest、远端版本和状态会写回 `mr-calligraphy-remote-publish-v1`。
