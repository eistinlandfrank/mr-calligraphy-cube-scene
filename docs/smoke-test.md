...
- 远端发布检查：模拟主后台和写实后台本机发布版本，确认 `MRProjectRemotePublish` 会拒绝未配置/非法 endpoint，保存 HTTP endpoint/token，生成带 manifest 和 SHA-256 摘要的发布包，携带 Bearer header，GET 检查远端，POST 当前发布包，并持久化 packageId、releaseId、packageDigest 和远端状态。
