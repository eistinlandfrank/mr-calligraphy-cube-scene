...
计划仓库、学习档案仓库、报告仓库、项目仓库和后台远端发布都已有 API 合同或 mock server、endpoint/token 配置、GET/PUT/POST 检查和本机状态持久化；计划仓库已补 Workspace 空间 ID，请求头、同步包、回执审计和 mock server 都能按空间隔离；计划仓库、报告仓库和项目仓库都能保存远端回执并导出本机 HTML 审计页；学习计划也能导出标准 `.ics` 日历提醒文件，便于导入系统日历或手机日历。这个方向是对的，但它目前证明的是“前端能对接 API / 本机能导出标准文件 / 本机能留存回执证据”，不是“项目已有生产后端”。
...
- `PlanRepository`、`HistoryRepository`、`ReportRepository`、`ProjectRepository` 服务端草案和最小实现；`PlanRepository` 已有 Workspace 空间隔离第一版，`ReportRepository` 与 `ProjectRepository` 远端 API adapter 第一版已完成，仍需真实账号空间、权限和服务端合并。
