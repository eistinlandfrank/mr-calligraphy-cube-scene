...
当前评分已经会读取真实笔迹点位，并输出结构、笔画、笔法、力度、流畅度等基础评分证据；本机 `ScoreService` adapter 会迁移旧练习评分记录，记录评分来源、算法版本、最近分数、最近证据摘要、累计评分次数和采样点，并在前台评分卡显示“本机基础评分”边界。本轮新增 `local-heuristic-v2.0.0` 评分证据包，包含永、仁、和、礼、雅、静、心的本机范字来源、目标笔顺、笔画差、压感采样点数和压感范围。但它仍是浏览器端启发式算法，不是训练模型，也不是硬件校准后的专业书法评级。

真实化方向：

- 已增加第一版范字笔顺数据、评分版本号、评分证据说明和 `ScoreService` adapter。
...
- `local-heuristic-v2.0.0` 已加入本机范字来源、目标笔顺、笔画差和压感采样证据。
