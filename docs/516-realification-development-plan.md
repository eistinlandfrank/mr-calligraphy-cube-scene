...
### 2026-06-13：新增报告对比导出回执审计

功能名：前台报告对比离线 HTML 导出回执与 HTML 审计导出。

开发原因：

- 站内报告对比页已经可以真实导出离线 HTML。
- 但用户导出后无法在报告详情中确认“导出的是哪两份报告、平均分差和文件摘要是什么”，也无法把对比导出记录单独留档。

完成内容：

- 新增 `mr-calligraphy-report-comparison-export-audit-v1` 审计包。
- 学习状态新增 `reportComparisonExportReceipts`，保存最近报告对比导出回执。
- `MRAppState.recordReportComparisonExportReceipt()` 记录上份报告、本份报告、平均分差、练习/作品/分钟变化、字段差值、文件摘要和回执摘要。
- `MRAppState.getReportComparisonExportAudit()` 返回提升/回落统计、回执列表和 `auditDigest`。
- `MRAppState.getReportComparisonExportAuditExport()` 生成 HTML 审计页。
- `MRAppState.downloadReportComparisonExportAudit()` 下载 `mr-calligraphy-report-comparison-export-audit-*.html`。
- `downloadReportComparison()` 写入报告对比导出回执。
- 前台报告详情面板新增“对比导出回执”面板和“导出回执”按钮。
- Smoke、状态层脚本和 Playwright 均新增覆盖。

验收方式：

- 在前台生成至少两份学习报告并打开最新报告详情。
- 点击“导出对比页”应下载 `mr-calligraphy-report-comparison-*.html`，并在“对比导出回执”面板出现前后报告回执。
- 回执应显示平均分变化、文件名、文件摘要和回执摘要。
- 点击“导出回执”应下载 HTML，文件包含“MR 书法报告对比导出回执审计”、两份报告 ID、文件摘要、回执摘要和边界说明。
- 刷新页面后仍可从 localStorage 按报告 ID 读取回执。

真实边界：

- 这是当前浏览器本机报告对比导出请求回执，只能证明页面生成并发起了 HTML 下载，并记录生成内容摘要；它不是操作系统保存完成证明、云端长期报告、跨设备下载日志、账号审计或不可篡改证据链。
...
### 2026-06-13：新增项目档案恢复审计导出回执审计

功能名：主后台项目档案恢复审计 HTML 导出回执。

背景：

- 项目档案恢复动作已经会写入 `mr-calligraphy-project-archive-audit-v1`，并支持导出恢复审计 HTML。
- 但恢复审计 HTML 被导出后，后台此前没有记录该文件本身的摘要、导出时间和恢复记录范围。
- 用户需要长期排查时，不仅要知道“恢复过什么”，还要知道“恢复审计报告何时被导出、导出的报告摘要是什么”。
...
- 主后台项目备份区新增“恢复审计导出回执”面板和“导出回执”按钮。
- Playwright 主后台用例覆盖恢复远端项目仓库版本、导出恢复审计、写入导出回执、持久化 localStorage 和下载 HTML 回执审计页。

手工验收：

- 在主后台拉取或导入一个项目档案，勾选恢复内容并点击“恢复所选”。
- 页面刷新后点击“恢复审计”的“导出审计”，应下载 `mr-calligraphy-archive-audit-*.html`。
- “恢复审计导出回执”面板应显示该文件名、恢复记录数、文件摘要和回执摘要。
- 点击“恢复审计导出回执”的“导出回执”，应下载 `mr-calligraphy-project-restore-audit-export-audit-*.html`，文件内包含恢复审计报告文件名、文件摘要、审计报告摘要和最近恢复记录摘要。
...
- 这是当前浏览器本机 HTML 下载请求回执，只能证明页面生成并发起了恢复审计报告下载，并记录生成内容摘要；它不是系统文件保存完成证明、账号审批、服务端归档或不可篡改日志。
...
### 2026-06-13：新增项目仓库包文件导入预览

功能名：主后台项目仓库包 JSON 文件导入恢复预览。

背景：

- 主后台已经可以下载与远端推送同结构的 `mr-calligraphy-project-repository-package-v1` JSON。
- 远端项目仓库拉取也能把同结构包转成恢复预览。
- 但用户下载到本机的项目仓库包此前不能通过“导入项目档案”入口回流，只能导出不能再导入，备份闭环不完整。

本轮完成：

- 新增 `readProjectImportFile()`，统一解析项目档案 JSON 与项目仓库包 JSON。
- 项目仓库包文件导入会校验 kind、version、archive 和 `packageDigest`。
- `prepareImportProject()` 识别项目仓库包后，把包内 archive 转成恢复预览。
- 恢复预览来源保留 packageId、Workspace、packageDigest、repositoryDigest、文件名和文件摘要。
- 主后台标题区显示“本机项目仓库包预览”，来源区显示“本机仓库包导入只生成恢复预览”。
- 差异报告导出回执新增 `project-repository-file` 来源类型，显示为“本机项目仓库包预览”。
- Playwright 主后台用例覆盖：导出项目仓库包、通过文件入口重新导入预览、篡改包未重签时拒绝进入恢复预览。

手工验收：

- 在主后台点击“导出仓库包”，下载 `mr-calligraphy-project-repository-package-*.json`。
- 通过“导入项目档案”选择该 JSON 文件。
- 页面应显示“本机项目仓库包预览”，来源区包含 packageId、Workspace 和包摘要。
- 修改该 JSON 中任意字段但不重算 `packageDigest` 后再次导入，应提示摘要不匹配并拒绝进入恢复预览。
...
- 这是本机 JSON 包导入恢复预览和摘要验真，不是账号化项目空间、多人合并、远端资产补齐、生产服务端签名或不可篡改审计。

提交：

- 中文 commit message：`新增仓库包文件导入预览`

### 2026-06-13：新增项目档案导入预览 JSON 导出

功能名：主后台恢复前导入预览 JSON 证据包。

背景：

- 项目档案导入预览已经能展示字段差异、模型差异、风险摘要和恢复选择。
- HTML 差异报告适合人工审阅，但不方便后续工具、服务端审批、长期比对或自动化排查读取。
- 需要在恢复前导出一份机器可读 JSON，把当前预览和勾选方案固定下来。

本轮完成：

- 主后台恢复预览操作区新增“导出预览 JSON”按钮。
- 新增 `getImportPreviewJsonExport()` 和 `downloadImportPreviewJson()`。
- 导出的 JSON 使用 `mr-calligraphy-project-import-preview-v1`。
- JSON 包含来源类型、档案来源、远端/本机仓库包来源、项目 schema 摘要、风险摘要、恢复选择、previewDigest、selectionDigest、exportDigest 和边界说明。
- 当前页面勾选的字段/模型恢复方案会进入 `restoreSelection`。
- Playwright 主后台用例覆盖本机项目仓库包预览后的 JSON 下载，并校验 kind、sourceType、packageId、Workspace、packageDigest、恢复选择和摘要字段。

手工验收：

- 在主后台导入项目档案或项目仓库包，生成恢复预览。
- 点击“导出预览 JSON”，应下载 `mr-calligraphy-import-preview-*.json`。
- JSON 中应包含 `previewDigest`、`selectionDigest`、`exportDigest` 和当前恢复选择。
- 未生成预览时按钮应禁用。
...
### 2026-06-13：新增项目档案导入预览 JSON 回执本机校验

功能名：主后台导入预览 JSON 导出回执 receiptDigest 本机重算校验。

背景：

- 导入预览 JSON 导出回执已经会记录 `receiptDigest`、`previewDigest`、`selectionDigest` 和 `exportDigest`。
- 但后台此前只展示摘要字符串，无法确认回执记录是否被手动篡改。
- 长期排查恢复前审阅证据时，需要至少能在当前浏览器内识别回执字段与摘要是否一致。

本轮完成：

- `getProjectImportPreviewExportAudit()` 读取回执时会重算 `receiptDigest`。
...
- 回执审计 HTML 新增“本机校验”和“重算摘要”。
- Playwright 主后台用例验证正常导入预览 JSON 导出回执校验通过，并临时篡改回执确认 `digest-mismatch` 被识别。

手工验收：

- 在主后台导入项目档案或项目仓库包，点击“导出预览 JSON”生成回执。
- “预览 JSON 回执”列表应显示“本机校验通过”。
- 点击“导出回执”下载 HTML，文件中应包含“本机校验通过”和“重算摘要”。
- 手动篡改 `mr-calligraphy-project-import-preview-export-audit-v1.records[0]` 任意声明字段后调用 `MRProjectArchive.getProjectImportPreviewExportAudit()`，应看到 `failedCount: 1` 和 `verificationStatus: "digest-mismatch"`。
...
- 这是当前浏览器本机摘要一致性校验，只能证明导入预览 JSON 导出回执与自身 `receiptDigest` 声明字段一致；它不是服务端签名、账号审批、生产证书链、远端不可篡改日志或多人审计链。

提交：

- 中文 commit message：`新增导入预览JSON回执校验`

### 2026-06-13：新增项目档案导出回执本机校验

功能名：主后台项目档案 JSON 导出回执 receiptDigest 本机重算校验。

背景：

- 项目档案导出回执已经会记录文件摘要、档案摘要和 `receiptDigest`。
- 但后台此前只展示摘要字符串，无法判断回执记录是否被手动篡改。
- 项目档案是恢复和迁移的基础入口，导出回执需要至少具备当前浏览器内的摘要一致性校验。

本轮完成：

- `getProjectArchiveExportAudit()` 读取回执时会重算 `receiptDigest`。
- 每条回执新增 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 回执审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 主后台“导出回执”列表显示本机校验状态。
- 项目档案导出回执 HTML 新增“本机校验”和“重算摘要”。
- Playwright 主后台用例验证正常项目档案导出回执校验通过，并临时篡改回执确认 `digest-mismatch` 被识别。

手工验收：

- 在主后台点击“导出项目档案”生成一条导出回执。
- “导出回执”列表应显示“本机校验通过”。
- 点击“导出回执”下载 HTML，文件中应包含“本机校验通过”和“重算摘要”。
- 手动篡改 `mr-calligraphy-project-archive-export-audit-v1.records[0]` 任意声明字段后调用 `MRProjectArchive.getProjectArchiveExportAudit()`，应看到 `failedCount: 1` 和 `verificationStatus: "digest-mismatch"`。
...
- 这是当前浏览器本机摘要一致性校验，只能证明项目档案导出回执与自身 `receiptDigest` 声明字段一致；它不是系统文件保存证明、云端备份完成证明、服务端签名、账号审批、生产证书链、远端不可篡改日志或多人审计链。
...
### 2026-06-13：新增项目档案差异报告回执本机校验

功能名：主后台项目档案差异报告 HTML 导出回执 receiptDigest 本机重算校验。

背景：

- 项目档案差异报告导出回执已经会记录文件摘要、预览摘要、恢复选择摘要和 `receiptDigest`。
- 但后台此前只展示摘要字符串，无法判断回执记录是否被手动篡改。
- 差异报告是恢复前审阅的关键证据，需要至少具备当前浏览器内的摘要一致性校验。

本轮完成：

- `getProjectImpactExportAudit()` 读取回执时会重算 `receiptDigest`。
- 每条回执新增 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 回执审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 主后台“差异报告导出回执”列表显示本机校验状态。
- 项目档案差异报告导出回执 HTML 新增“本机校验”和“重算摘要”。
- Playwright 主后台用例验证正常项目档案差异报告导出回执校验通过，并临时篡改回执确认 `digest-mismatch` 被识别。

手工验收：

- 在主后台导入项目档案或拉取远端项目仓库版本，生成恢复预览后点击“导出差异报告”。
- “差异报告导出回执”列表应显示“本机校验通过”。
- 点击“导出回执”下载 HTML，文件中应包含“本机校验通过”和“重算摘要”。
- 手动篡改 `mr-calligraphy-project-impact-export-audit-v1.records[0]` 任意声明字段后调用 `MRProjectArchive.getProjectImpactExportAudit()`，应看到 `failedCount: 1` 和 `verificationStatus: "digest-mismatch"`。
...
- 这是当前浏览器本机摘要一致性校验，只能证明项目档案差异报告导出回执与自身 `receiptDigest` 声明字段一致；它不是恢复动作证明、服务端签名、账号审批、生产证书链、远端不可篡改日志或多人审计链。

提交：

- 中文 commit message：`新增差异报告回执校验`

### 2026-06-13：新增项目档案恢复审计导出回执本机校验

功能名：主后台项目档案恢复审计 HTML 导出回执 receiptDigest 本机重算校验。

背景：

- 项目档案恢复审计导出回执已经会记录文件摘要、审计报告摘要、最近恢复记录摘要和 `receiptDigest`。
- 但后台此前只展示摘要字符串，无法判断回执记录是否被手动篡改。
- 恢复审计导出 HTML 是恢复结果对外留档的关键证据，需要至少具备当前浏览器内的摘要一致性校验。

本轮完成：

- `getProjectRestoreAuditExportAudit()` 读取回执时会重算 `receiptDigest`。
- 每条回执新增 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 回执审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 主后台“恢复审计导出回执”列表显示本机校验状态。
- 项目档案恢复审计导出回执 HTML 新增“本机校验”和“重算摘要”。
- Playwright 主后台用例验证正常项目档案恢复审计导出回执校验通过，并临时篡改回执确认 `digest-mismatch` 被识别。

手工验收：

- 在主后台完成一次项目档案恢复后点击“导出审计”。
- “恢复审计导出回执”列表应显示“本机校验通过”。
- 点击“导出回执”下载 HTML，文件中应包含“本机校验通过”和“重算摘要”。
- 手动篡改 `mr-calligraphy-project-restore-audit-export-v1.records[0]` 任意声明字段后调用 `MRProjectArchive.getProjectRestoreAuditExportAudit()`，应看到 `failedCount: 1` 和 `verificationStatus: "digest-mismatch"`。
...
- 这是当前浏览器本机摘要一致性校验，只能证明项目档案恢复审计导出回执与自身 `receiptDigest` 声明字段一致；它不是系统文件保存证明、恢复动作审批、服务端签名、生产证书链、远端不可篡改日志或多人审计链。

提交：

- 中文 commit message：`新增恢复审计导出回执校验`

### 2026-06-13：新增报告导出回执本机校验

功能名：前台报告 HTML/PDF 导出回执 receiptDigest 本机重算校验。

背景：

- 报告导出回执已经会记录报告验真摘要、文件摘要、文件大小和 `receiptDigest`。
- 但前台此前只展示摘要字符串，无法判断回执记录是否被手动篡改。
- 报告 HTML/PDF 是用户最容易拿去留档或分享的导出物，导出回执需要至少具备当前浏览器内的摘要一致性校验。

本轮完成：

- `getReportExportAudit()` 读取回执时会重算 `receiptDigest`。
- 每条回执新增 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 回执审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 前台“报告导出回执”列表显示本机校验状态。
- 报告导出回执 HTML 新增“本机校验”和“重算摘要”。
- Playwright 前台报告用例验证正常报告 HTML/PDF 导出回执校验通过，并临时篡改回执确认 `digest-mismatch` 被识别。

手工验收：

- 在前台生成报告后点击“下载 HTML”和“下载 PDF”。
- “报告导出回执”列表应显示“本机校验通过”。
- 点击“导出回执”下载 HTML，文件中应包含“本机校验通过”和“重算摘要”。
- 手动篡改 `mr-calligraphy-learning-state-v1.reportExportReceipts[0]` 任意声明字段后刷新页面并调用 `MRAppState.getReportExportAudit(reportId)`，应看到 `failedCount: 1` 和 `verificationStatus: "digest-mismatch"`。
...
- 这是当前浏览器本机摘要一致性校验，只能证明报告导出回执与自身 `receiptDigest` 声明字段一致；它不是操作系统保存完成证明、云端 PDF 渲染日志、账号下载审计、服务端签名、生产证书链、远端不可篡改日志或多人审计链。

提交：

- 中文 commit message：`新增报告导出回执校验`

### 2026-06-13：新增报告对比导出回执本机校验

功能名：前台报告对比 HTML 导出回执 receiptDigest 本机重算校验。

背景：

- 报告对比导出回执已经会记录前后报告 ID、平均分差、文件摘要、文件大小和 `receiptDigest`。
- 但前台此前只展示摘要字符串，无法判断回执记录是否被手动篡改。
- 报告对比 HTML 是用户复盘阶段变化的离线证据，导出回执需要至少具备当前浏览器内的摘要一致性校验。

本轮完成：

- `getReportComparisonExportAudit()` 读取回执时会重算 `receiptDigest`。
- 每条回执新增 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 回执审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 前台“报告对比导出回执”列表显示本机校验状态。
- 报告对比导出回执 HTML 新增“本机校验”和“重算摘要”。
- Playwright 前台报告用例验证正常报告对比导出回执校验通过，并临时篡改回执确认 `digest-mismatch` 被识别。

手工验收：

- 在前台进入有上一份报告的报告详情，点击“导出报告对比”。
- “报告对比导出回执”列表应显示“本机校验通过”。
- 点击“导出回执”下载 HTML，文件中应包含“本机校验通过”和“重算摘要”。
- 手动篡改 `mr-calligraphy-learning-state-v1.reportComparisonExportReceipts[0]` 任意声明字段后刷新页面并调用 `MRAppState.getReportComparisonExportAudit(reportId)`，应看到 `failedCount: 1` 和 `verificationStatus: "digest-mismatch"`。
...
- 这是当前浏览器本机摘要一致性校验，只能证明报告对比导出回执与自身 `receiptDigest` 声明字段一致；它不是云端长期报告、跨设备下载日志、账号审计、服务端签名、生产证书链、远端不可篡改日志或多人审计链。
...
### 2026-06-13：新增计划导出回执本机校验

功能名：前台学习计划 HTML / 日历 ICS 导出回执 receiptDigest 本机重算校验。

背景：

- 计划导出回执已经会记录计划 ID、导出类型、文件名、任务数量、完成数量、进度、日历事件数量、文件摘要和 `receiptDigest`。
- 但此前前台只展示回执摘要，无法判断 localStorage 里的导出回执是否被手动篡改。
- 计划 HTML 和 ICS 是用户长期保存学习计划的离线文件，至少需要在当前浏览器内明确“回执字段和摘要是否一致”。

本轮完成：

- `getPlanExportAudit()` 读取回执时会重算 `receiptDigest`。
- 每条回执新增 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 回执审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 前台“计划导出回执”列表显示本机校验状态。
- 计划导出回执 HTML 新增“本机校验”和“重算摘要”。
- Playwright 前台报告用例验证正常计划导出回执校验通过，并临时篡改回执确认 `digest-mismatch` 被识别。

手工验收：

- 在前台生成学习计划后分别点击“导出计划”和“导出日历”。
- “计划导出回执”列表应显示“本机校验通过”。
- 点击“导出回执”下载 HTML，文件中应包含“本机校验通过”和“重算摘要”。
- 手动篡改 `mr-calligraphy-learning-state-v1.planExportReceipts[0]` 任意声明字段后刷新页面并调用 `MRAppState.getPlanExportAudit(planId)`，应看到 `failedCount: 1` 和 `verificationStatus: "digest-mismatch"`。
...
- 这是当前浏览器本机摘要一致性校验，只能证明计划导出回执与自身 `receiptDigest` 声明字段一致；它不是操作系统保存完成证明、云端下载日志、账号审计、服务端签名、生产证书链、远端不可篡改日志或多人审计链。

提交：

- 中文 commit message：`新增计划导出回执校验`

### 2026-06-13：新增计划仓库导出回执本机校验

功能名：前台学习计划仓库 JSON 同步包导出回执 receiptDigest 本机重算校验。

背景：

- 计划仓库导出回执已经会记录文件名、字节数、文件摘要、同步包 ID、包摘要、计划数量、Workspace 和 `receiptDigest`。
- 但此前前台只展示回执摘要，无法判断 localStorage 里的计划仓库导出回执是否被手动篡改。
- 计划仓库 JSON 同步包是跨设备迁移和远端同步前的关键文件，至少需要在当前浏览器内明确“回执字段和摘要是否一致”。

本轮完成：

- `getPlanRepositoryExportAudit()` 读取回执时会重算 `receiptDigest`。
- 每条回执新增 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 回执审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 前台“计划仓库导出回执”列表显示本机校验状态。
- 计划仓库导出回执 HTML 新增“本机校验”和“重算摘要”。
- Playwright 计划仓库用例验证正常计划仓库导出回执校验通过，并临时篡改回执确认 `digest-mismatch` 被识别。

手工验收：

- 在前台生成学习计划后点击“导出同步包”。
- “计划仓库导出回执”列表应显示“本机校验通过”。
- 点击“导出回执”下载 HTML，文件中应包含“本机校验通过”和“重算摘要”。
- 手动篡改 `mr-calligraphy-learning-state-v1.planRepositoryExportReceipts[0]` 任意声明字段后刷新页面并调用 `MRAppState.getPlanRepositoryExportAudit()`，应看到 `failedCount: 1` 和 `verificationStatus: "digest-mismatch"`。
...
- 这是当前浏览器本机摘要一致性校验，只能证明计划仓库导出回执与自身 `receiptDigest` 声明字段一致；它不是云端仓库写入证明、跨设备下载日志、账号审计、服务端签名、生产证书链、远端不可篡改日志或多人审计链。
...
### 2026-06-13：新增学习档案详情操作回执本机校验

功能名：前台学习档案详情图片下载、报告下载和直达链接复制回执 receiptDigest 本机重算校验。

背景：

- 学习档案详情操作回执已经会记录操作类型、目标记录、文件名、链接、文件或链接摘要、复制状态、记录时间和 `receiptDigest`。
- 但此前前台只展示回执摘要，无法判断 localStorage 里的详情操作回执是否被手动篡改。
- 详情页操作会影响用户对学习档案下载和分享链路的信任，至少需要在当前浏览器内明确“回执字段和摘要是否一致”。

本轮完成：

- `getHistoryDetailActionAudit()` 读取回执时会重算 `receiptDigest`。
- 每条回执新增 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 回执审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 前台“详情操作回执”列表显示本机校验状态。
- 学习档案详情操作回执 HTML 新增“本机校验”和“重算摘要”。
- Playwright 前台报告用例验证正常详情操作回执校验通过，并临时篡改链接回执确认 `digest-mismatch` 被识别。

手工验收：

- 在前台打开一条学习档案详情，下载图片、下载报告或复制直达链接。
- “详情操作回执”列表应显示“本机校验通过”。
- 点击“导出审计”下载 HTML，文件中应包含“本机校验通过”和“重算摘要”。
- 手动篡改 `mr-calligraphy-learning-state-v1.historyDetailActionReceipts[0]` 任意声明字段后刷新页面并调用 `MRAppState.getHistoryDetailActionAudit()`，应看到 `failedCount: 1` 和 `verificationStatus: "digest-mismatch"`。
...
- 这是当前浏览器本机摘要一致性校验，只能证明详情操作回执与自身 `receiptDigest` 声明字段一致；它不是操作系统下载完成证明、剪贴板系统审计、云端访问日志、账号审计、服务端签名、生产证书链、远端不可篡改日志或多人审计链。
...
### 2026-06-13：新增学习档案仓库导出回执本机校验

功能名：前台学习档案 JSON 同步包导出回执 receiptDigest 本机重算校验。

背景：

- 学习档案仓库导出回执已经会记录文件名、MIME、字节数、文件摘要、包 ID、包摘要、Workspace、练习/作品/报告/阶段数量和 `receiptDigest`。
- 但此前前台只展示回执摘要，无法判断 localStorage 里的学习档案仓库导出回执是否被手动篡改。
- 学习档案同步包是跨设备迁移和远端同步前的关键本机资产，至少需要在当前浏览器内明确“回执字段和摘要是否一致”。

本轮完成：

- `getHistoryRepositoryExportAudit()` 读取回执时会重算 `receiptDigest`。
- 每条回执新增 `verificationStatus`、`verificationMessage` 和 `verificationExpectedDigest`。
- 回执审计摘要新增 `verifiedCount`、`failedCount` 和 `legacyCount`。
- 前台“学习档案仓库导出回执”列表显示本机校验状态。
- 学习档案仓库导出回执 HTML 新增“本机校验”和“重算摘要”。
- Playwright 学习档案仓库用例验证正常导出回执校验通过，并临时篡改文件名确认 `digest-mismatch` 被识别。

手工验收：

- 在前台学习档案区域点击“导出同步包”。
- “学习档案仓库导出回执”列表应显示“本机校验通过”。
- 点击“导出回执”下载 HTML，文件中应包含“本机校验通过”和“重算摘要”。
- 手动篡改 `mr-calligraphy-learning-state-v1.historyRepositoryExportReceipts[0]` 任意声明字段后刷新页面并调用 `MRAppState.getHistoryRepositoryExportAudit()`，应看到 `failedCount: 1` 和 `verificationStatus: "digest-mismatch"`。
...
- 这是当前浏览器本机摘要一致性校验，只能证明学习档案仓库导出回执与自身 `receiptDigest` 声明字段一致；它不是操作系统保存完成证明、云端下载日志、账号审计、服务端签名、生产证书链、远端不可篡改日志或多人审计链。
