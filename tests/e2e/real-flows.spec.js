...
test("front practice saves real strokes and exports a report", async ({ page }) => {
  ...
  await page.locator("#actionList .action-button").filter({ hasText: /^开始听$/ }).click();
  ...
  await page.locator("#actionList .action-button").filter({ hasText: /^练字$/ }).click();
  ...
  await page.locator("#actionList .action-button").filter({ hasText: /^比一比$/ }).click();
  ...
  await page.locator("#actionList .action-button").filter({ hasText: /^看笔画$/ }).click();
  ...
  await page.locator("#actionList .action-button").filter({ hasText: /^下一笔$/ }).click();
  ...
  await page.locator("#actionList .action-button").filter({ hasText: /^写作品$/ }).click();
  await expect(page.locator("#sceneTitle")).toContainText("写作品");
  await expect(page.locator("#actionFeedback")).toContainText("创作实践");
  await expect(page.locator("#actionDetail")).toContainText("本机阶段记录");
  await expect(page.locator("#actionDetail")).toContainText("阶段记录 ID");

  await page.locator("#actionList .action-button").filter({ hasText: /^保存作品$/ }).click();
  ...
  await expect(page.locator("#historyDetailActionAuditStatus")).toContainText("暂无学习档案详情操作回执");
  await expect(page.locator("#historyDetailDownloadImage")).toBeEnabled();
  const historyImageDownloadPromise = page.waitForEvent("download");
  await page.locator("#historyDetailDownloadImage").click();
  const historyImageDownload = await historyImageDownloadPromise;
  expect(historyImageDownload.suggestedFilename()).toMatch(/\.jpg$/);
  await expect(page.locator("#historyDetailActionAuditStatus")).toContainText("1 条学习档案详情操作回执");
  await expect(page.locator("#historyDetailActionAuditList")).toContainText("详情图片下载");
  await page.locator("#historyDetailCopyLink").click();
  await expect(page.locator("#noticeState")).toContainText(/已复制这条学习档案的直达链接|写入地址栏/);
  await expect(page.locator("#historyDetailActionAuditStatus")).toContainText("2 条学习档案详情操作回执");
  await expect(page.locator("#historyDetailActionAuditList")).toContainText("详情直达链接复制");
  learningState = await readJsonLocalStorage(page, LEARNING_KEY);
  expect(learningState.historyDetailActionReceipts).toHaveLength(2);
  expect(learningState.historyDetailActionReceipts.map((receipt) => receipt.actionType)).toEqual(["link-copy", "image-download"]);
  ...
  expect(historyActionAuditDownload.suggestedFilename()).toMatch(/^mr-calligraphy-history-detail-action-audit-.*\.html$/);
  const historyActionAuditPath = await historyActionAuditDownload.path();
  const historyActionAuditHtml = fs.readFileSync(historyActionAuditPath, "utf8");
  expect(historyActionAuditHtml).toContain("MR 书法学习档案详情操作回执审计");
  ...
  await page.locator("#actionList .action-button").filter({ hasText: /^导报告$/ }).click();
  ...
  await expect(page.locator("#historyDetail")).toBeVisible();
  await expect(page.locator("#historyDetailType")).toContainText("报告详情");
  await expect(page.locator("#historyDetailActionAuditStatus")).toContainText("暂无学习档案详情操作回执");
  const historyReportDownloadPromise = page.waitForEvent("download");
  await page.locator("#historyDetailDownloadReport").click();
  const historyReportDownload = await historyReportDownloadPromise;
  expect(historyReportDownload.suggestedFilename()).toMatch(/^mr-calligraphy-report-.*\.html$/);
  await expect(page.locator("#historyDetailActionAuditStatus")).toContainText("1 条学习档案详情操作回执");
  await expect(page.locator("#historyDetailActionAuditList")).toContainText("详情报告 HTML 下载");
  learningState = await readJsonLocalStorage(page, LEARNING_KEY);
  expect(learningState.historyDetailActionReceipts[0].actionType).toBe("report-download");
  expect(learningState.historyDetailActionReceipts[0].reportId).toBe(reportId);
  expect(learningState.historyDetailActionReceipts[0].artifactDigest).toMatch(/^[a-f0-9]{64}$/);
  const historyReportActionAuditDownloadPromise = page.waitForEvent("download");
  await page.locator("#historyDetailActionAuditExport").click();
  const historyReportActionAuditDownload = await historyReportActionAuditDownloadPromise;
  const historyReportActionAuditPath = await historyReportActionAuditDownload.path();
  ...
  await page.locator("#actionList .action-button").filter({ hasText: /^做计划$/ }).click();
  ...
  await page.locator("#actionList .action-button").filter({ hasText: /^看详情$/ }).click();
  ...
  await page.locator("#actionList .action-button").filter({ hasText: /^再练笔$/ }).click();
  await expect(page.locator("#sceneTitle")).toContainText("看笔画");
...
test("front artwork repository exports and imports local artwork package", async ({ page }) => {
  ...
  const repositoryRedownloadPromise = page.waitForEvent("download");
  await page.locator("#artworkRepositoryExportButton").click();
  const repositoryRedownload = await repositoryRedownloadPromise;
  expect(repositoryRedownload.suggestedFilename()).toMatch(/^mr-calligraphy-artwork-repository-.*\.json$/);
  const repositoryRedownloadPackage = JSON.parse(fs.readFileSync(await repositoryRedownload.path(), "utf8"));
  expect(repositoryRedownloadPackage.artworks).toHaveLength(3);
  expect(repositoryRedownloadPackage.packageDigest).toMatch(/^[a-f0-9]{64}$/);
  await expect(page.locator("#artworkExportAuditStatus")).toContainText("1 条作品导出回执");
  await expect(page.locator("#artworkExportAuditList")).toContainText("作品仓库 JSON");
  ...
  expect(artworkAuditHtml).toContain("作品仓库 JSON");
  ...
  expect(resolvedState.artworkExportReceipts).toHaveLength(4);
