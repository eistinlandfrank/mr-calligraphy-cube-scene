(function () {
  ...
  function normalizeReportExportReceipts(records) {
    const source = Array.isArray(records) ? records : [];
    const seen = new Set();
    return source
      .map(normalizeReportExportReceipt)
      .filter(Boolean)
  ...
  function normalizeReportExportReceipt(record) {
    ...
    if (!exportType || !reportId || !exportedAt || !filename) return null;
    const fileDigest = normalizeReportTeacherReviewDigest(record.fileDigest || record.contentDigest || record.digest);
    const reportDigest = normalizeReportTeacherReviewDigest(record.reportDigest || record.verificationDigest);
    const byteLength = normalizeInteger(record.byteLength, 0, 0, 999999999);
    const id = String(record.id || `report-export-${sha256StableJson({
      exportType,
      reportId,
      filename,
      exportedAt,
    ...
    const payload = {
      ...
      id: id.slice(0, 120),
      exportType,
      exportLabel: formatReportExportTypeLabel(exportType),
      reportId,
      reportTitle: String(record.reportTitle || record.title || reportId || "学习报告").trim().slice(0, 140) || "学习报告",
      reportCreatedAt: normalizePlanDate(record.reportCreatedAt) || "",
      reportDigest,
      filename,
      mimeType: String(record.mimeType || getReportExportMimeType(exportType)).trim().slice(0, 120),
      byteLength,
      fileDigest,
      exportedAt,
      source: String(record.source || getReportExportSource(exportType)).trim().slice(0, 80),
      features: record.features && typeof record.features === "object" ? clone(record.features) : {},
      boundary: String(record.boundary || REPORT_EXPORT_AUDIT_BOUNDARY).trim().slice(0, 300) || REPORT_EXPORT_AUDIT_BOUNDARY,
    ...
    if (!payload.message) {
      payload.message = `已记录报告“${payload.reportTitle}”的${payload.exportLabel}导出回执。`;
    }
    return payload;
  }
  ...
  function normalizeReportExportType(value) {
    ...
    if (["html", "report-html", "learning-report-html"].includes(type)) return "report-html";
    if (["pdf", "report-pdf", "learning-report-pdf"].includes(type)) return "report-pdf";
    return "";
  }

  function formatReportExportTypeLabel(type) {
    return {
      "report-html": "报告 HTML",
      "report-pdf": "原生 PDF"
    }[type] || "报告导出";
  ...
  function getReportExportMimeType(type) {
    return type === "report-pdf"
      ? "application/pdf"
      : "text/html;charset=utf-8";
  }

  function getReportExportSource(type) {
    return {
  ...
  function addReportComparisonExportReceiptVerification(record) {
    const normalized = normalizeReportComparisonExportReceipt(record);
    if (!normalized) {
      return null;
    }
    const verification = verifyReportComparisonExportReceiptDigest(normalized);
    return {
      ...normalized,
      verificationStatus: verification.status,
      verificationMessage: verification.message,
  ...
  function verifyReportComparisonExportReceiptDigest(record = {}) {
    const receipt = normalizeReportComparisonExportReceipt(record);
    const receiptDigest = normalizeReportTeacherReviewDigest(receipt?.receiptDigest);
    if (!receipt || !receiptDigest) {
      return {
        status: "legacy",
        expectedDigest: "",
        message: "旧报告对比导出回执未生成 receiptDigest，无法执行本机一致性校验。"
      };
    }
    const expectedDigest = sha256StableJson(createReportComparisonExportReceiptDigestPayload(receipt));
    const status = expectedDigest === receiptDigest ? "verified" : "digest-mismatch";
    return {
      status,
      expectedDigest,
      message: status === "verified"
        ? "本机一致性校验通过：receiptDigest 与报告对比导出回执声明字段一致。"
        : "本机一致性校验失败：receiptDigest 无法按报告对比导出回执声明字段重算匹配。"
    };
  }

  function createReportComparisonExportReceiptDigestPayload(record = {}) {
    const receipt = normalizeReportComparisonExportReceipt(record) || {};
    return {
      kind: REPORT_COMPARISON_EXPORT_AUDIT_KIND,
      currentReportId: receipt.currentReportId || "",
      previousReportId: receipt.previousReportId || "",
      averageDelta: normalizeInteger(receipt.averageDelta, 0, -1000, 1000),
      filename: receipt.filename || "",
      mimeType: receipt.mimeType || "text/html;charset=utf-8",
      byteLength: normalizeInteger(receipt.byteLength, 0, 0, 999999999),
      fileDigest: receipt.fileDigest || "",
      exportedAt: receipt.exportedAt || ""
    };
  ...
  function addReviewExportReceiptVerification(record) {
    const normalized = normalizeReviewExportReceipt(record);
    if (!normalized) {
      return null;
    }
    const verification = verifyReviewExportReceiptDigest(normalized);
    return {
      ...normalized,
      verificationStatus: verification.status,
      verificationMessage: verification.message,
  ...
  function verifyReviewExportReceiptDigest(record = {}) {
    const receipt = normalizeReviewExportReceipt(record);
    const receiptDigest = normalizeReportTeacherReviewDigest(receipt?.receiptDigest);
    if (!receipt || !receiptDigest) {
      return {
        status: "legacy",
        expectedDigest: "",
        message: "旧复盘导出回执未生成 receiptDigest，无法执行本机一致性校验。"
      };
    }
    const expectedDigest = sha256StableJson(createReviewExportReceiptDigestPayload(receipt));
    const status = expectedDigest === receiptDigest ? "verified" : "digest-mismatch";
    return {
      status,
      expectedDigest,
      message: status === "verified"
        ? "本机一致性校验通过：receiptDigest 与复盘导出回执声明字段一致。"
        : "本机一致性校验失败：receiptDigest 无法按复盘导出回执声明字段重算匹配。"
    };
  }

  function createReviewExportReceiptDigestPayload(record = {}) {
    const receipt = normalizeReviewExportReceipt(record) || {};
    return {
      kind: REVIEW_EXPORT_AUDIT_KIND,
      exportType: receipt.exportType || "",
...
<body>
  ...
  function addPlanRepositoryExportReceiptVerification(record) {
    const normalized = normalizePlanRepositoryExportReceipt(record);
    if (!normalized) {
      return null;
    }
    const verification = verifyPlanRepositoryExportReceiptDigest(normalized);
    return {
      ...normalized,
      verificationStatus: verification.status,
      verificationMessage: verification.message,
  ...
  function verifyPlanRepositoryExportReceiptDigest(record = {}) {
    const receipt = normalizePlanRepositoryExportReceipt(record);
    const receiptDigest = normalizeReportTeacherReviewDigest(receipt?.receiptDigest);
    if (!receipt || !receiptDigest) {
      return {
        status: "legacy",
        expectedDigest: "",
        message: "旧计划仓库导出回执未生成 receiptDigest，无法执行本机一致性校验。"
      };
    }
    const expectedDigest = sha256StableJson(createPlanRepositoryExportReceiptDigestPayload(receipt));
    const status = expectedDigest === receiptDigest ? "verified" : "digest-mismatch";
    return {
      status,
      expectedDigest,
      message: status === "verified"
        ? "本机一致性校验通过：receiptDigest 与计划仓库导出回执声明字段一致。"
        : "本机一致性校验失败：receiptDigest 无法按计划仓库导出回执声明字段重算匹配。"
    };
  }

  function createPlanRepositoryExportReceiptDigestPayload(record = {}) {
    const receipt = normalizePlanRepositoryExportReceipt(record) || {};
    return {
      kind: PLAN_REPOSITORY_EXPORT_AUDIT_KIND,
      filename: receipt.filename || "",
      byteLength: normalizeInteger(receipt.byteLength, 0, 0, 200000000),
      fileDigest: receipt.fileDigest || "",
      packageId: receipt.packageId || "",
      packageDigest: receipt.packageDigest || "",
      planCount: normalizeInteger(receipt.planCount, 0, 0, 9999),
      workspaceId: receipt.workspaceId || PLAN_REPOSITORY_DEFAULT_WORKSPACE,
      exportedAt: receipt.exportedAt || ""
    };
  }
  ...
  function getPlanRepositoryExportAudit(options = {}) {
    ...
    const allReceipts = normalizePlanRepositoryExportReceipts(state.planRepositoryExportReceipts)
      .map(addPlanRepositoryExportReceiptVerification)
      .filter(Boolean);
    ...
    const audit = {
      ...
      message: allReceipts.length
        ? `已记录 ${allReceipts.length} 条计划仓库导出回执，本机校验通过 ${verifiedCount} 条${failedCount ? `，失败 ${failedCount} 条` : ""}${legacyCount ? `，旧记录 ${legacyCount} 条` : ""}。最近一次：${formatPlanDate(allReceipts[0].exportedAt)}。`
        : "暂无计划仓库导出回执。"
    };
    audit.auditDigest = sha256StableJson({
      ...audit,
    ...
    const rows = audit.receipts.map((receipt) => `
      <section class="receipt">
        ...
        <dl>
          ...
          <div><dt>文件摘要</dt><dd>${escapeHtml(receipt.fileDigest || "未生成")}</dd></div>
          <div><dt>回执摘要</dt><dd>${escapeHtml(receipt.receiptDigest || "未生成")}</dd></div>
          <div><dt>本机校验</dt><dd>${escapeHtml(formatPlanRepositoryExportReceiptVerificationStatus(receipt.verificationStatus))}</dd></div>
          <div><dt>重算摘要</dt><dd>${escapeHtml(receipt.verificationExpectedDigest || "无法重算")}</dd></div>
          <div><dt>导出时间</dt><dd>${escapeHtml(formatDateTime(receipt.exportedAt))}</dd></div>
        </dl>
      </section>`).join("");
...
<body>
  <main>
    ...
    <p class="meta">导出 ${escapeHtml(audit.exportedCount)} / ${escapeHtml(audit.total)} 条计划仓库导出回执，本机校验通过 ${escapeHtml(audit.verifiedCount || 0)} 条${audit.failedCount ? `，失败 ${escapeHtml(audit.failedCount)} 条` : ""}${audit.legacyCount ? `，旧记录 ${escapeHtml(audit.legacyCount)} 条` : ""}。${escapeHtml(audit.boundary)}</p>
    ${rows}
    <h2>原始审计 JSON</h2>
    <pre>${escapeHtml(JSON.stringify(audit, null, 2))}</pre>
    <footer>审计摘要：${escapeHtml(audit.auditDigest)}。本机校验通过：${escapeHtml(audit.verifiedCount || 0)}，失败：${escapeHtml(audit.failedCount || 0)}，旧记录：${escapeHtml(audit.legacyCount || 0)}。数据来源：${escapeHtml(audit.storageKey)}。导出时间：${escapeHtml(formatDateTime(exportedAt))}。</footer>
...
<body>
  ...
  function recordReportExportReceipt(reportId = null, options = {}) {
    const detail = getReportDetail(reportId);
    if (!detail) {
      return { ok: false, message: "还没有可导出的站内报告。" };
    }
    const exportType = normalizeReportExportType(options.exportType || options.type);
    if (!exportType) {
      return { ok: false, message: "报告导出回执缺少有效导出类型。" };
    }
    const filename = String(options.filename || "").trim();
    if (!filename) {
      return { ok: false, message: "报告导出回执缺少文件名。" };
    }
    const verification = getReportVerification(detail.id);
    const content = String(options.content ?? options.html ?? options.pdf ?? "");
    ...
    const receipt = normalizeReportExportReceipt({
      id: options.id || makeId("report-export"),
      exportType,
      reportId: detail.id,
      reportTitle: detail.title || "学习报告",
      reportCreatedAt: detail.createdAt,
      reportDigest: options.reportDigest || verification?.digest || "",
      filename,
      mimeType: options.mimeType || getReportExportMimeType(exportType),
      byteLength,
      fileDigest: options.fileDigest || (content ? sha256Hex(content) : ""),
      exportedAt: options.exportedAt || new Date().toISOString(),
      source: options.source || getReportExportSource(exportType),
      features: options.features || {},
      boundary: REPORT_EXPORT_AUDIT_BOUNDARY,
      message: options.message || `已记录报告“${detail.title || "学习报告"}”的${formatReportExportTypeLabel(exportType)}导出回执。`
    });
    if (!receipt) {
      return { ok: false, message: "报告导出回执记录失败。" };
    }
    state.reportExportReceipts = [
      receipt,
      ...normalizeReportExportReceipts(state.reportExportReceipts).filter((item) => item.id !== receipt.id && item.receiptDigest !== receipt.receiptDigest)
    ].slice(0, REPORT_EXPORT_MAX_RECEIPTS);
    addEvent("report-export", `${formatReportExportTypeLabel(exportType)}：${receipt.reportTitle}`);
    saveState();
    return {
      ok: true,
      receipt: clone(receipt),
      audit: getReportExportAudit(detail.id),
      message: receipt.message
    };
  }

  function getReportExportAudit(reportId = null, options = {}) {
    const targetReportId = String(reportId || "").trim();
  ...
  function getReportExportAuditExport(reportId = null, options = {}) {
    const audit = getReportExportAudit(reportId, options);
    if (!audit.total) {
      return {
        ok: false,
        audit,
        message: audit.message || "暂无可导出的报告导出回执。"
      };
    }
    const exportedAt = new Date().toISOString();
    const reportSlug = audit.reportId ? makeDownloadSlug(audit.reportId) : "all";
    return {
      ok: true,
      filename: `mr-calligraphy-report-export-audit-${reportSlug}-${exportedAt.slice(0, 10)}.html`,
      html: renderReportExportAuditHtml(audit, exportedAt),
      audit,
      message: `已生成 ${audit.exportedCount} 条报告导出回执审计导出。`
    };
  }

  function downloadReportExportAudit(reportId = null, options = {}) {
    const result = getReportExportAuditExport(reportId, options);
    if (!result.ok) {
      return result;
    }
    downloadHtml(result.html, result.filename);
  ...
  function renderReportExportAuditHtml(audit, exportedAt) {
    const badges = Object.entries(audit.typeCounts || {})
      .map(([type, count]) => `<span>${escapeHtml(formatReportExportTypeLabel(type))} ${escapeHtml(count)}</span>`)
      .join("") || "<span>暂无类型统计</span>";
    const rows = audit.receipts.map((receipt) => `
      <section class="receipt">
        <h2>${escapeHtml(receipt.exportLabel || formatReportExportTypeLabel(receipt.exportType))} · ${escapeHtml(receipt.reportTitle || receipt.reportId)}</h2>
        <p>${escapeHtml(receipt.message || "报告导出回执已记录。")}</p>
        <dl>
          <div><dt>报告 ID</dt><dd>${escapeHtml(receipt.reportId)}</dd></div>
          <div><dt>报告摘要</dt><dd>${escapeHtml(receipt.reportDigest || "未生成")}</dd></div>
          <div><dt>文件名</dt><dd>${escapeHtml(receipt.filename)}</dd></div>
...
<body>
  ...
  function getReportComparisonExportAudit(reportId = null, options = {}) {
    ...
    const allReceipts = normalizeReportComparisonExportReceipts(state.reportComparisonExportReceipts)
      .map(addReportComparisonExportReceiptVerification)
      .filter(Boolean);
    const filtered = allReceipts.filter((receipt) => (
    ...
    const audit = {
      ...
      boundary: REPORT_COMPARISON_EXPORT_AUDIT_BOUNDARY,
      message: filtered.length
        ? `已记录 ${filtered.length} 条报告对比导出回执，本机校验通过 ${verifiedCount} 条${failedCount ? `，失败 ${failedCount} 条` : ""}${legacyCount ? `，旧记录 ${legacyCount} 条` : ""}。最近一次：${formatPlanDate(filtered[0].exportedAt)}。`
        : targetReportId
          ? "当前报告暂无对比导出回执记录。"
          : "暂无报告对比导出回执记录。"
    };
    ...
    const rows = audit.receipts.map((receipt) => {
      ...
      <section class="receipt">
        ...
        <dl>
          ...
          <div><dt>文件摘要</dt><dd>${escapeHtml(receipt.fileDigest || "未生成")}</dd></div>
          <div><dt>回执摘要</dt><dd>${escapeHtml(receipt.receiptDigest || "未生成")}</dd></div>
          <div><dt>本机校验</dt><dd>${escapeHtml(formatReportComparisonExportReceiptVerificationStatus(receipt.verificationStatus))}</dd></div>
          <div><dt>重算摘要</dt><dd>${escapeHtml(receipt.verificationExpectedDigest || "无法重算")}</dd></div>
          <div><dt>文件大小</dt><dd>${escapeHtml(receipt.byteLength || 0)} bytes</dd></div>
          <div><dt>导出时间</dt><dd>${escapeHtml(formatDateTime(receipt.exportedAt))}</dd></div>
        </dl>
...
<body>
  <main>
    ...
    <p class="meta">导出 ${escapeHtml(audit.exportedCount)} / ${escapeHtml(audit.total)} 条报告对比导出回执，本机校验通过 ${escapeHtml(audit.verifiedCount || 0)} 条${audit.failedCount ? `，失败 ${escapeHtml(audit.failedCount)} 条` : ""}${audit.legacyCount ? `，旧记录 ${escapeHtml(audit.legacyCount)} 条` : ""}。${escapeHtml(audit.boundary)}</p>
    ${rows}
    <h2>原始审计 JSON</h2>
    <pre>${escapeHtml(JSON.stringify(audit, null, 2))}</pre>
    <footer>审计摘要：${escapeHtml(audit.auditDigest)}。本机校验通过：${escapeHtml(audit.verifiedCount || 0)}，失败：${escapeHtml(audit.failedCount || 0)}，旧记录：${escapeHtml(audit.legacyCount || 0)}。数据来源：${escapeHtml(audit.storageKey)}。导出时间：${escapeHtml(formatDateTime(exportedAt))}。</footer>
...
<body>
  ...
  function getReviewExportAudit(options = {}) {
    ...
    const allReceipts = normalizeReviewExportReceipts(state.reviewExportReceipts)
      .map(addReviewExportReceiptVerification)
      .filter(Boolean);
    const filtered = allReceipts.filter((receipt) => (
    ...
    const audit = {
      ...
      boundary: REVIEW_EXPORT_AUDIT_BOUNDARY,
      message: filtered.length
        ? `已记录 ${filtered.length} 条复盘导出回执，本机校验通过 ${verifiedCount} 条${failedCount ? `，失败 ${failedCount} 条` : ""}${legacyCount ? `，旧记录 ${legacyCount} 条` : ""}。最近一次：${formatPlanDate(filtered[0].exportedAt)}。`
        : "暂无复盘导出回执。"
    };
    audit.auditDigest = sha256StableJson({
      ...audit,
    ...
    const rows = audit.receipts.map((receipt) => `
      <section class="receipt">
        ...
        <dl>
          ...
          <div><dt>文件摘要</dt><dd>${escapeHtml(receipt.fileDigest || "未生成")}</dd></div>
          <div><dt>回执摘要</dt><dd>${escapeHtml(receipt.receiptDigest || "未生成")}</dd></div>
          <div><dt>本机校验</dt><dd>${escapeHtml(formatReviewExportReceiptVerificationStatus(receipt.verificationStatus))}</dd></div>
          <div><dt>重算摘要</dt><dd>${escapeHtml(receipt.verificationExpectedDigest || "无法重算")}</dd></div>
...
<body>
  <main>
    ...
    <h1>MR 书法复盘导出回执审计</h1>
    <p class="meta">导出 ${escapeHtml(audit.exportedCount)} / ${escapeHtml(audit.total)} 条复盘导出回执，本机校验通过 ${escapeHtml(audit.verifiedCount || 0)} 条${audit.failedCount ? `，失败 ${escapeHtml(audit.failedCount)} 条` : ""}${audit.legacyCount ? `，旧记录 ${escapeHtml(audit.legacyCount)} 条` : ""}。${escapeHtml(audit.boundary)}</p>
    <div class="badges">${badges}</div>
    ${rows}
    <h2>原始审计 JSON</h2>
    <pre>${escapeHtml(JSON.stringify(audit, null, 2))}</pre>
...
<body>
  ...
  function downloadReport(reportId = null) {
    ...
    const exportReceiptResult = recordReportExportReceipt(result.report?.id || reportId, {
      exportType: "report-html",
      filename: result.filename,
      mimeType: result.mimeType || "text/html;charset=utf-8",
      content: result.html,
      reportDigest: result.verification?.digest || "",
      features: result.features || {},
      source: "report-html-download"
    });
    return {
      ok: true,
      filename: result.filename,
      receipt: receiptResult.ok ? receiptResult.receipt : null,
      exportReceipt: exportReceiptResult.ok ? exportReceiptResult.receipt : null,
      audit: receiptResult.ok ? receiptResult.audit : null,
      exportAudit: exportReceiptResult.ok ? exportReceiptResult.audit : null,
      message: `已下载${reportId ? "所选" : "最近"} HTML 学习报告，含能力雷达、签名水印、教师批注状态和打印样式，并记录复盘导出回执和报告导出回执。`
    };
  }
  ...
  function downloadReportPdf(reportId = null) {
    ...
    const receiptResult = recordReportExportReceipt(result.report?.id || reportId, {
    ...
    return {
      ...
      message: receiptResult.ok
        ? `已下载原生 PDF 学习报告：${result.filename}，并记录报告导出回执。`
        : `已下载原生 PDF 学习报告：${result.filename}。`
    };
  }
